...
export function DockIconButton({
	...
	const surface = row
		? danger
			? "border-transparent bg-transparent text-red-400 hover:bg-red-500/10"
			: active
				? "border-transparent bg-transparent text-white hover:bg-white/[0.06]"
	...
	return (
		...
		>
			<span
				className={`relative size-full border [border-radius:inherit] [transition-property:background,color,border-color,transform,opacity] duration-[110ms] [transition-timing-function:cubic-bezier(0.23,1,0.32,1)] group-active/icon-action:scale-[0.96] motion-reduce:transition-none motion-reduce:group-active/icon-action:scale-100 ${layout} ${surface} ${row ? "!border-transparent" : ""}`}
			>
				{children}
				{badge !== undefined && (
					<span
