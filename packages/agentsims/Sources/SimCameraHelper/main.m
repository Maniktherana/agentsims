// SimCameraHelper — host-side source manager for agentsims's simulator
...
//
// Command line:
//   agentsims-camera-helper --shm <name> [--socket <path>]
//                           [--source placeholder|webcam|image]
//                           [--arg <value>]   # webcam name / image path
//                           [--width 1280] [--height 720]
//   agentsims-camera-helper --list
//
...
static void StartPlaceholderSource(void) {
    ...
    if (!buf) {
        fprintf(stderr, "[agentsims-camera] placeholder buf alloc failed (%zu bytes)\n", need);
        return;
    }
    ...
    fprintf(stderr, "[agentsims-camera] placeholder source running @ 30fps (%ux%u, first frame seq=%llu)\n",
...
static BOOL StartWebcamSource(NSString *deviceArg, NSString **err) {
    ...
    [sess startRunning];
    gWebcamSession = sess;
    fprintf(stderr, "[agentsims-camera] webcam → %s\n", device.localizedName.UTF8String);
    return YES;
}
...
static BOOL StartImageSource(NSString *path, NSString **err) {
    ...
    PublishFrame(buf);
    free(buf);
    fprintf(stderr, "[agentsims-camera] image → %s\n", path.UTF8String);
    return YES;
}
...
static void RunVideoLoop(NSString *path) {
    ...
    if (tracks.count == 0) {
        fprintf(stderr, "[agentsims-camera] video → %s: no video tracks\n", path.UTF8String);
    ...
    while (!atomic_load(&gVideoCancelled)) {
        ...
        if (!out) {
            fprintf(stderr, "[agentsims-camera] video reader failed: %s\n", err.UTF8String ?: "?");
            break;
        }
        ...
        if (status == AVAssetReaderStatusFailed) {
            fprintf(stderr, "[agentsims-camera] video reader failed mid-loop: %s\n",
...
static BOOL StartVideoSource(NSString *path, NSString **err) {
    ...
    if (!gVideoQueue) {
        gVideoQueue = dispatch_queue_create("agentsims.cam.video", DISPATCH_QUEUE_SERIAL);
    }
    ...
    fprintf(stderr, "[agentsims-camera] video → %s\n", path.UTF8String);
    return YES;
}
...
#pragma clang diagnostic pop
    for (uint32_t i = 0; i < SIMCAM_SURFACE_RING; i++) {
        ...
        if (!s) {
            fprintf(stderr, "[agentsims-camera] IOSurfaceCreate failed at %u\n", i);
            return NO;
        }
        gSurfaces[i] = s;
...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        if (k == (SimCamSourceKind)-1) {
            fprintf(stderr, "[agentsims-camera] unknown --source %s, defaulting to placeholder\n",
                initialSource.UTF8String);
            k = SimCamSourcePlaceholder;
        }
        NSString *err = nil;
        if (!SwitchSource(k, initialArg, &err)) {
            fprintf(stderr, "[agentsims-camera] initial source failed: %s — falling back to placeholder\n",
                err.UTF8String ?: "?");
        ...
        if (socketPath) {
            if (OpenControlSocket(socketPath) < 0) {
                fprintf(stderr, "[agentsims-camera] control socket open failed: %s\n", socketPath);
            } else {
                fprintf(stderr, "[agentsims-camera] control socket %s\n", socketPath);
            }
        }
        ...
        fprintf(stderr, "[agentsims-camera] running — Ctrl+C to stop\n");
        while (!gShouldExit) {
        ...
        fprintf(stderr, "[agentsims-camera] stopped\n");
        return 0;
    }
}
