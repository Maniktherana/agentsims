#!/bin/bash
# Builds agentsims-native.node — the in-process N-API addon that replaces the
# spawned helper. The JS bindings are written in Swift with
...
PKG="$(cd "$HERE/../.." && pwd)"          # packages/agentsims (Package.swift root)
OUT_DIR="${1:-$PKG/dist/native}"
BUILD_DIR="$PKG/.build"
PRODUCT="agentsims-native"
mkdir -p "$OUT_DIR"
