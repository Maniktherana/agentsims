...
const PNG_CACHE_ROOT = join(tmpdir(), "agentsims-devicekit-chrome");
const PLACEHOLDER_ASSET_CACHE_ROOT = join(
	tmpdir(),
	"agentsims-device-placeholder-assets",
);
...
const placeholderAssetInfoCache = new Map<
	string,
	Promise<PlaceholderAssetInfo | null>
>();
...
export function parsePdfPageSize(pdf: Buffer | string): Size | null {
	...
	const box =
		/\/CropBox\s*\[\s*(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)\s*\]/.exec(
			text,
		) ??
...
async function resolveDevicePlaceholderAssetUncached(device: {
	...
	return info
		? { name: iconName, width: info.width, height: info.height }
		: null;
}
...
async function placeholderAssetInfo(
	name: string,
): Promise<PlaceholderAssetInfo | null> {
...
async function placeholderAssetSourcePath(
	name: string,
): Promise<string | null> {
	...
	if (!(await coreTypesIconEntries()).some((entry) => entry.iconName === name))
		return null;
	const path = join(MOBILE_DEVICE_RESOURCES_ROOT, `${name}.icns`);
	return existsSync(path) ? path : null;
}

function fallbackPlaceholderAsset(
	name: string,
): PlaceholderAssetDefinition | null {
...
async function resolveDeviceFrameUncached(
	...
	const frame: Size = {
		width:
			resolvedBodySize.width +
			chrome.devicePadding.left +
			chrome.devicePadding.right,
		height:
			resolvedBodySize.height +
			chrome.devicePadding.top +
			chrome.devicePadding.bottom,
	};
	...
	const buttons = chrome.buttons.flatMap((button): DeviceFrameButton[] => {
		...
		const hover = {
			x:
				imageSize.width > 0
					? (button.rolloverOffset.x - button.normalOffset.x) / imageSize.width
					: 0,
			y:
				imageSize.height > 0
					? (button.rolloverOffset.y - button.normalOffset.y) / imageSize.height
					: 0,
		};
		return [
			{
...
async function iconNameForProfile(
...
function fallbackIconNameForDeviceName(name: string): string | null {
	...
	if (/ipad\s+air\s+11-inch\s+\(m4\)/.test(normalized))
		return "ipad-air-11-inch-m4";
	if (/ipad\s+air\s+13-inch\s+\(m4\)/.test(normalized))
...
async function coreTypesIconEntries(): Promise<CoreTypesIconEntry[]> {
	...
	coreTypesIconEntriesCache = (async () => {
		const info =
			(await readPlist(
				join(MOBILE_DEVICE_RESOURCES_ROOT, "MobileDevices-Info.plist"),
			)) ??
			(await readPlist(
				join(dirname(MOBILE_DEVICE_RESOURCES_ROOT), "Info.plist"),
			));
...
function parseChromeJson(json: JsonRecord): ParsedChrome | null {
	const identifierValue =
		typeof json.identifier === "string" ? json.identifier : null;
	...
	const outerCornerRadius = numberValue(
		record(paths.simpleOutsideBorder).cornerRadiusX,
	);
	const insets = {
	...
	const compositeImage =
		typeof images.composite === "string" ? images.composite : null;
	...
	if (Array.isArray(json.inputs)) {
		for (const input of json.inputs) {
			...
			if (typeof entry.imageDown === "string")
				allowedImages.add(entry.imageDown);
		}
	}
	...
	return {
		...
		devicePadding,
		outerCornerRadius,
		innerCornerRadius: Math.max(
			outerCornerRadius - Math.max(insets.left, insets.top),
			0,
		),
		compositeImage,
		slice,
	...
	switch (button.anchor) {
		case "left": {
			...
			return {
				...
				y: bodyY + button.rolloverOffset.y,
			};
		}
		case "right":
...
async function deviceTypeNameForIdentifier(
...
async function readPlist(path: string): Promise<JsonRecord | null> {
	try {
		const json = await hostCommandText(
			"plutil",
			"-convert",
			"json",
			"-o",
 ...
 */
async function compositeScreenBounds(
	identifier: string,
	imageName: string,
): Promise<(Rect & { radius: number }) | null> {
	const png = await readCompositePng(identifier, imageName);
	if (!png) return null;
	const decoded = decodeMask(
		png,
		(r, g, b, a) => a > 200 && r < 30 && g < 30 && b < 30,
	);
