...
export function exposeDeviceState(
  ...
  if (!proxy) {
    ...
    if (hostname === "localhost" || hostname === "127.0.0.1" || hostname === "[::1]") return state;
    const rewrite = (value: string) => value.replace("127.0.0.1", hostname);
    return { ...state, url: rewrite(state.url), streamUrl: rewrite(state.streamUrl), wsUrl: rewrite(state.wsUrl) };
  }
