...
function withPublicPortHeader(firstChunk: Buffer, publicPort: number): Buffer {
  const headEnd = firstChunk.indexOf("\r\n\r\n");
  if (headEnd === -1) return firstChunk;
  const header = Buffer.from(`\r\nx-agentsims-public-port: ${publicPort}`, "ascii");
  return Buffer.concat([
    firstChunk.subarray(0, headEnd),
    header,
    firstChunk.subarray(headEnd),
  ]);
}

function proxyTcpToHttpServer(socket: Socket, firstChunk: Buffer, port: number, publicPort: number): void {
  ...
  upstream.on("connect", () => {
    upstream.write(withPublicPortHeader(firstChunk, publicPort));
...
function createPreviewFrontServer(
  ...
  return createNetServer((socket) => {
    ...
    const onData = (chunk: Buffer) => {
      ...
      const pathname = new URL(parsed.url, "http://agentsims.local").pathname;
      ...
      proxyTcpToHttpServer(socket, buffered, internalPort, publicPort);
    };
...
export async function servePreview(opts: {
  ...
  if (isBun) {
    ...
    const frontServer = createPreviewFrontServer(opts.middleware, internalAddress.port, opts.port);
