	...
	try {
		...
		if (parsed.devices && typeof parsed.devices === "object") {
			for (const [deviceId, route] of Object.entries(parsed.devices)) {
				...
				const inputDeviceId =
					typeof route.inputDeviceId === "string"
						? route.inputDeviceId
						: undefined;
				const outputDeviceId =
					typeof route.outputDeviceId === "string"
						? route.outputDeviceId
						: undefined;
				const androidCameraFront =
					typeof route.androidCameraFront === "string"
						? route.androidCameraFront
						: undefined;
				const androidCameraBack =
					typeof route.androidCameraBack === "string"
						? route.androidCameraBack
						: undefined;
				devices[deviceId] = {
					inputDeviceId,
					outputDeviceId,
