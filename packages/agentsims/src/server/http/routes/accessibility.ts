...
export const accessibilityRoutes = HttpRouter.empty.pipe(
  HttpRouter.get("/source", Effect.gen(function*() {
    ...
    if (!testID || !file || !Number.isInteger(line) || line < 1) {
      return HttpServerResponse.raw(json({ error: "Missing source identity" }, 400));
    }
    const source = readRnSourceFile({ testID, file, line });
    if (!source) return HttpServerResponse.raw(json({ error: "Source unavailable" }, 404));
    const etag = JSON.stringify(source.cacheKey);
    if (request.headers.get("if-none-match") === etag) {
      return HttpServerResponse.raw(new Response(null, { status: 304, headers: { ETag: etag } }));
    }
    return HttpServerResponse.raw(new Response(JSON.stringify(source), {
      headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "private, no-cache", ETag: etag },
    }));
  })),
  HttpRouter.post("/ax/refresh", Effect.gen(function*() {
  ...
  HttpRouter.get("/ax", Effect.gen(function*() {
    const runtime = yield* HttpRuntime;
    const request = requestSource((yield* HttpServerRequest.HttpServerRequest).source);
    const url = new URL(request.url);
    const states = yield* runtime.readStates;
    ...
    const updates = Stream.asyncScoped<Uint8Array>((emit) =>
      Effect.acquireRelease(
        Effect.sync(() => runtime.streamers.get(state.device).addClient({
          write: (chunk) => { void emit.single(encoder.encode(chunk)); },
        })),
