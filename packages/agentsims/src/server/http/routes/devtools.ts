...
export const devtoolsRoutes = HttpRouter.empty.pipe(
	HttpRouter.get(
		...
		Effect.gen(function* () {
			...
			const targets = (yield* Effect.promise(() => bridge.listTargets())).map(
				(target) => {
					const encoded = encodeURIComponent(target.id);
					const frontend = new URL(
						`${runtime.basePath}/devtools-frontend/inspector.html`,
						request.url,
					);
	...
	HttpRouter.get(
		...
		Effect.gen(function* () {
			...
			const bridge = yield* runtime.getBridge;
			return HttpServerResponse.raw(
				json(yield* Effect.promise(() => bridge.listTargets())),
			);
		}),
	),
	HttpRouter.get(
	...
	HttpRouter.get(
		...
		Effect.gen(function* () {
			...
			const upstream = yield* Effect.promise(() =>
				fetch(
					`https://chrome-devtools-frontend.appspot.com/serve_rev/@${FRONTEND_REVISION}/${asset}${new URL(request.url).search}`,
				),
			);
			return HttpServerResponse.raw(
