import { HttpApiBuilder, HttpServerRequest, HttpServerResponse, type HttpApp } from "@effect/platform";
import { BunContext, BunHttpServer } from "@effect/platform-bun";
import { Effect, Layer, ManagedRuntime } from "effect";
import { ShellExecLive } from "../../services/runtime";
import { httpRuntimeLayer, type HttpServerOptions } from "../../services/http-runtime";
...
const jsonContentTypeGuard = (app: HttpApp.Default) => Effect.gen(function*() {
  ...
  if (source instanceof Request && source.method === "POST") {
    ...
    if (requiresJson && !source.headers.get("content-type")?.startsWith("application/json")) {
...
export async function servePreview(options: HttpServerOptions & {
  host: string;
  port: number;
