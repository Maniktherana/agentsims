...
import { androidSerialFromStateId, getAndroidForegroundApp } from "../android/device/device";
import { getDeviceSession } from "../ios/session/session";
import { decodeForegroundApp, type ForegroundApp } from "../shared/foreground-app";
import { createAxStreamerCache, type AxStreamerCache } from "../accessibility/snapshot";
import { deviceCommands, type DeviceCommands } from "../commands/device-commands";
import { MediaCommands, type MediaOperations } from "../commands/media-commands";
import type { DeviceState } from "../shared/state";
import { readDeviceStates } from "../server/devices/device-lifecycle";
import { ensureInspectWebKitBridge, type WebKitBridge } from "../server/http/devtools-bridge";
...
export type HttpServerOptions = {
  ...
  deviceCommands?: Pick<DeviceCommands, "list" | "memory" | "workspaces" | "observe" | "act" | "start" | "shutdown">;
...
export type HttpRuntimeService = Omit<
  HttpServerOptions,
  "basePath" | "axStreamers" | "readDeviceStates" | "readForegroundApp" | "deviceCommands" | "mediaOperations" | "getBridge" | "saveScreenshot"
> & {
  basePath: "" | `/${string}`;
  ...
  commands: Pick<DeviceCommands, "list" | "memory" | "workspaces" | "observe" | "act" | "start" | "shutdown">;
  media: MediaCommands;
...
async function readForegroundApp(device: string): Promise<ForegroundApp | null> {
...
export function httpRuntimeLayer(options: HttpServerOptions): Layer.Layer<HttpRuntime> {
  ...
  return Layer.succeed(HttpRuntime, {
    ...
    foregroundApp: (device) => Effect.promise(() =>
      (options.readForegroundApp ?? readForegroundApp)(device)
    ),
    commands: options.deviceCommands ?? deviceCommands,
    media: new MediaCommands(options.mediaOperations ?? new MediaRouter(basePath)),
