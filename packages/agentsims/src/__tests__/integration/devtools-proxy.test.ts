  ...
  test("lists targets and bridges CDP text frames", async () => {
    ...
    cdp = Bun.serve({
      ...
      fetch(request, server) {
        if (new URL(request.url).pathname.startsWith("/devtools/page/") && server.upgrade(request)) return undefined;
    ...
    const bridge: WebKitBridge = {
      ...
      async listTargets() {
        return [{ id: targetId, title: "Example", url: "https://example.test", type: "page", udid }];
      },
    };
    ...
    const response = await fetch(`${started.origin}/devtools?device=${encodeURIComponent(udid)}`);
    expect(response.status).toBe(200);
    const body: unknown = await response.json();
    if (!body || typeof body !== "object" || !("targets" in body) || !Array.isArray(body.targets)) {
    ...
    const echoed = await new Promise<string>((resolve, reject) => {
      ...
      const timer = setTimeout(() => reject(new Error("CDP echo timeout")), 2_000);
