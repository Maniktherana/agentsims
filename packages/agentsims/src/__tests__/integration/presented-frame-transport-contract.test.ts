...
const simulatorView = readFileSync(
	join(import.meta.dir, "../../web/components/simulator/simulator-view.tsx"),
	"utf8",
);
const avccStream = readFileSync(
	join(import.meta.dir, "../../web/hooks/simulator/use-avcc-stream.ts"),
	"utf8",
);
...
describe("presented-frame transport contract", () => {
	...
	test("does not run a permanent MJPEG paint animation loop", () => {
		...
		expect(simulatorView.match(/requestAnimationFrame\(paint\)/g)).toHaveLength(
			1,
		);
