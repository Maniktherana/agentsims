...
import { reconcileStreamingDeviceVisibility } from "../web/workspace/use-device-workspace";
...
describe("workspace streaming state", () => {
  ...
  test("preserves referential equality when visibility has not changed", () => {
    ...
    expect(reconcileStreamingDeviceVisibility(current, ["ios-one", "ios-two"])).toBe(current);
  });
});
