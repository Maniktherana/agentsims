...
async function loadFfi(): Promise<NonNullable<typeof lib>> {
	...
	const handle = dlopen("libSystem.dylib", {
		shm_open: {
			args: [FFIType.cstring, FFIType.i32, FFIType.u16],
			returns: FFIType.i32,
		},
		mmap: {
			args: [
	...
	lib = {
		...
		munmap: (addr, len) =>
			Number(handle.symbols.munmap(addr as never, len as never) as number),
		close: (fd) => Number(handle.symbols.close(fd) as number),
		shm_unlink: (name) =>
			Number(handle.symbols.shm_unlink(name as never) as number),
...
describeIf("SimCameraHelper shm probe", () => {
	...
	beforeAll(async () => {
		...
		const stderr = (
			helper as unknown as { stderr: NodeJS.ReadableStream | null }
		).stderr;
	...
	test("publishes a shm region with the documented header layout", async () => {
		...
		try {
			...
			expect(header.pixelByteSize).toBe(
				BigInt(DEFAULT_WIDTH * DEFAULT_HEIGHT * 4),
			);
		} finally {
	...
	test("latestIndex stays in range as frames advance", async () => {
		...
		try {
			...
			const moved = await waitFor(() => {
				...
				return (
					header.frameSeq !== initialFrameSeq && t.latestIndex < t.surfaceCount
				);
			}, 1000);
	...
	test("dimensions and aspect ratio survive a SwitchSource call", async () => {
		await sendHelperCommand(SOCKET_PATH, {
			action: "switch",
			source: "placeholder",
		});
		const handle = await openExistingShm(SHM_NAME);
