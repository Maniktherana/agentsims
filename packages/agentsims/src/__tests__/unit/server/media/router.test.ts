...
function androidStatus(serial = "emulator-5554"): AndroidStatus {
	return {
		...
		audio: {
			hostRoute: serial.startsWith("emulator-")
				? "emulator-default"
				: "device-default",
			canChangeLive: false,
		},
...
describe("media routing model", () => {
	test("targets media requests to the device named by the panel URL", () => {
		...
		expect(
			mediaDeviceFromRequestUrl("/media?device=android%3Aemulator-5554"),
...
`).mediaVolume,
		...
		expect(androidMediaVolumeKeyEvents(5, 15, 0, 15)).toEqual(
			Array(10).fill("24"),
		);
		expect(androidMediaVolumeKeyEvents(10, 5, 0, 15)).toEqual(
			Array(5).fill("25"),
		);
	...
	test("gates Android image360 camera mode by emulator version", () => {
		expect(
			parseAndroidEmulatorVersion(
				"INFO | Android emulator version 36.6.4.0 (build_id 123)",
			),
		).toBe("36.6.4.0");
	...
	test("keeps front and back camera source validation platform-accurate", () => {
		...
		expect(
			validateAndroidCameraStartupMode("back", "imagefile:/tmp/camera.png"),
		).toBe(true);
		expect(
			validateAndroidCameraStartupMode("back", "videofile:/tmp/camera.mov"),
		).toBe(true);
		expect(
			validateAndroidCameraStartupMode("back", "image360:/tmp/pano.jpg"),
		).toBe(true);
		expect(validateAndroidCameraStartupMode("back", "videoplayback")).toBe(
			false,
		);
	});
	...
	test("exposes live microphone and restart-bound webcam controls for emulators", () => {
		...
		expect(state.audioOutput.volumeLevel).toEqual({
			current: 5,
			min: 0,
			max: 15,
		});
		...
		expect(
			state.camera.frontChoices.find((choice) => choice.id === "webcam0"),
		).toEqual({
			id: "webcam0",
		...
		expect(
			state.camera.backChoices.find((choice) => choice.id === "image360:")
				?.apply,
		).toBe("device-restart");
	});
	...
	test("reports iOS camera injection with Mac-wide actual and preferred audio routes", () => {
		...
		expect(state.camera.supportsFiles).toBe(true);
		expect(state.camera.sourceChoices?.map((choice) => choice.id)).toContain(
			"image",
		);
		expect(state.camera.sourceChoices?.map((choice) => choice.id)).toContain(
			"video",
