...
import { DeviceObservationCommands } from "../../../commands/device-observation";
...
describe("DeviceObservationCommands", () => {
  test("captures settled accessibility before screenshot and config", async () => {
    ...
    const observation = await Effect.runPromise(commands.observe("android:emulator-5554"));
  ...
  test("returns the screenshot when accessibility is unavailable", async () => {
    ...
    const observation = await Effect.runPromise(commands.observe("ios-device"));
