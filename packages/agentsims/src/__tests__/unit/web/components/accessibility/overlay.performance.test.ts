...
} from "../../../../../web/components/accessibility/overlay";
import {
	...
	axTargetVisualStyle,
	AxTarget,
} from "../../../../../web/components/accessibility/target";
...
describe("AX overlay render bounds", () => {
	...
	test("keeps hidden Android nodes in the raw set but excludes them from overlay eligibility", () => {
		...
		expect(
			result.visibleEntries.map((candidate) => candidate.element.id),
		).toContain("hidden-action");
		expect(
			result.eligibleEntries.map((candidate) => candidate.element.id),
		).toEqual(["legacy-action"]);
		expect(
			buildAxOverlayTargetEntries(elements, screen, {
				actionableOnly: false,
			}).eligibleEntries.map((candidate) => candidate.element.id),
		).toEqual(["title", "legacy-action"]);
	});
	...
	test("projects a Pressable chip as one actionable target, not its visual children", () => {
		...
		expect(result.previewKeyByRawKey.get("settings-icon@0.0.0")).toBe(
			"settings-chip@0.0",
		);
		expect(result.previewKeyByRawKey.get("settings-label@0.0.1")).toBe(
			"settings-chip@0.0",
		);
		...
		expect(
			selectRenderedAxTargetEntries(result.eligibleEntries, {
				interactive: false,
				inspecting: true,
				showAllOutlines: false,
				highlightedKey: null,
		...
		).toEqual(["settings-chip@0.0"]);
		expect(result.previewKeyByRawKey.get("settings-info@0.0.2")).toBe(
			"settings-info@0.0.2",
		);
		expect(
			projectAxOverlayTargetKeys(
	...
	test("collapses an exact-bound actionable View into its semantic Button descendant", () => {
		...
		expect(result.previewKeyByRawKey.get("view-action@0.0")).toBe(
			"button-action@0.0.0",
		);
		expect(result.previewKeyByRawKey.get("button-action@0.0.0")).toBe(
			"button-action@0.0.0",
		);
	...
	test("collapses one-coordinate AX rounding drift for the same nested action", () => {
		...
		expect(result.previewKeyByRawKey.get("rounded-view@0.0")).toBe(
			"rounded-button@0.0.0",
		);
	});
	...
	test("does not transitively collapse a two-coordinate nested drift chain", () => {
		...
		expect(result.previewKeyByRawKey.get("drift-outer@0.0")).toBe(
			"drift-outer@0.0",
		);
		expect(result.previewKeyByRawKey.get("drift-middle@0.0.0")).toBe(
			"drift-button@0.0.0.0",
		);
		expect(result.previewKeyByRawKey.get("drift-button@0.0.0.0")).toBe(
			"drift-button@0.0.0.0",
		);
	});
	...
	test("keeps unrelated overlapping actions even when their bounds match", () => {
		...
		expect(result.previewKeyByRawKey.get("first-action@0.0")).toBe(
			"first-action@0.0",
		);
		expect(result.previewKeyByRawKey.get("second-action@0.1")).toBe(
			"second-action@0.1",
		);
	});
	...
	test("keeps independently actionable nested controls with distinct bounds", () => {
		...
		expect(result.previewKeyByRawKey.get("card-action@0.0")).toBe(
			"card-action@0.0",
		);
		expect(result.previewKeyByRawKey.get("menu-action@0.0.0")).toBe(
			"menu-action@0.0.0",
		);
	});
});
