...
import type { ExecResult } from "../../../../web/simulator/input/exec";
...
function decodeChunkCommand(command: string): {
	bytes: Uint8Array;
	op: string;
} {
...
describe("uploadDroppedFile", () => {
	test("chunked writes reconstruct the original file, then addmedia + cleanup", async () => {
		...
		expect(
			commands.some((c) => c.startsWith("xcrun simctl addmedia UDID ")),
		).toBe(true);
	...
	test("ipa drops install instead of addmedia", async () => {
		...
		expect(
			commands.some((c) => c.startsWith("xcrun simctl install UDID ")),
		).toBe(true);
		expect(commands.some((c) => c.includes("addmedia"))).toBe(false);
	});

	test("Android media drops use adb push and notify the media scanner", async () => {
		const file = new File([patternBytes(64)], "shot.png", {
			type: "image/png",
		});
		const commands: string[] = [];
		await uploadDroppedFile(
		...
		expect(
			commands.some(
				(command) =>
					command.startsWith("adb -s 'emulator-5554' push ") &&
					command.includes("/sdcard/Pictures/"),
			),
		).toBe(true);
		expect(
			commands.some((command) =>
				command.includes("android.intent.action.MEDIA_SCANNER_SCAN_FILE"),
			),
		).toBe(true);
		expect(commands.some((command) => command.includes("xcrun simctl"))).toBe(
			false,
		);
	});

	test("Android rejects iOS application packages", async () => {
		const file = new File([patternBytes(64)], "app.ipa", { type: "" });
		await expect(
			uploadDroppedFile(
				file,
		...
		).rejects.toThrow("IPA files can only be installed on iOS simulators");
	});
