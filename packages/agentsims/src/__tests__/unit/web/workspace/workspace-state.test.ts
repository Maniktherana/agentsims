...
describe("workspace selection state", () => {
	test("reconciles a disappeared live Android serial to its shutdown AVD catalog row", () => {
		...
		const shutdownGrid = [{ device: "android-avd:Pixel_Tablet", helper: null }];
		...
		expect(effectiveDeviceId(state, [], "android:emulator-5556")).toBe(
			"android-avd:Pixel_Tablet",
		);
		expect(
			subscribedWorkspaceDeviceIds([], state.selectedDeviceId, false),
		).toEqual([]);
	});
	...
	test("effective selection falls back through visible device then config", () => {
		...
		expect(
			effectiveDeviceId(createWorkspaceSelectionState(null), [], "ios-2"),
		).toBe("ios-2");
	});

	test("subscribes once per visible device and adds only a separately focused helper", () => {
		expect(
			subscribedWorkspaceDeviceIds(["ios-1", "android-1"], "ios-1", true),
		).toEqual(["ios-1", "android-1"]);
