...
} from "../../../../../web/simulator/stream/avcc-codec.js";
...
describe("AvccDemuxer", () => {
	...
	test("demuxes and validates exact Android presentation metadata", () => {
		...
		expect(parseAndroidFramePresentation(chunk.payload)).toEqual({
			generation: 2,
		});
		expect(
			parseAndroidFramePresentation(
				new TextEncoder().encode(JSON.stringify({ generation: 0 })),
			),
		).toBeNull();
	});
	...
	test("reassembles a large keyframe delivered in tiny pieces", () => {
		...
		for (let off = 0; off < f.length; off += 64) {
			chunks = chunks.concat(
				d.push(f.subarray(off, Math.min(off + 64, f.length))),
			);
		}
		expect(chunks).toHaveLength(1);
		expect(chunks[0]!.type).toBe("keyframe");
	...
	perfTest("accumulation cost stays near-linear as chunking gets finer", () => {
		...
		const run = (piece: number) => {
			...
			for (let off = 0; off < stream.length; off += piece) {
				n += d.push(
					stream.subarray(off, Math.min(off + piece, stream.length)),
				).length;
			}
			return n;
		};
