...
import { createMjpegFrameParser } from "../../../../../web/simulator/stream/mjpeg-frame-parser";
...
describe("createMjpegFrameParser", () => {
	...
	test("header-less: large frames reassemble byte-perfect under fine chunking", () => {
		...
		const { stream, frames } = buildStream(
			3,
			32 * 1024,
			/* withHeaders */ false,
		);
	...
	perfTest(
		"accumulation cost stays near-linear (header path)",
		nearLinear(true),
	);
	perfTest(
		"accumulation cost stays near-linear (header-less scan path)",
		nearLinear(false),
	);
});
