...
describe("AppConfig", () => {
  test("resolves environment defaults behind one service", async () => {
    const config = await Effect.runPromise(resolveAppConfig({}, {
      HOST: "0.0.0.0",
      PORT: "3210",
    }));
  ...
  test("rejects invalid environment ports", async () => {
    await expect(Effect.runPromise(resolveAppConfig({}, { PORT: "invalid" }))).rejects.toThrow(
  ...
  test("supports a stub layer without process environment mutation", async () => {
    const program = Effect.gen(function*() {
      ...
      Effect.provide(appConfigLayer({ port: 9000 }, {})),
    );
