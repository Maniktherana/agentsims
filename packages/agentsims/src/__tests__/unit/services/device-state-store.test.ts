...
import { DeviceStateStore, deviceStateStoreLayer } from "../../../services/device-state-store";

test("DeviceStateStore uses platform filesystem services", async () => {
  const directory = mkdtempSync(join(tmpdir(), "agentsims-state-"));
  const state = { pid: 1, port: 3200, device: "ios", url: "http://x", streamUrl: "http://x/s", wsUrl: "ws://x" };
  try {
    ...
    }).pipe(Effect.provide(deviceStateStoreLayer(directory, 1).pipe(Layer.provide(BunContext.layer)))));
