...
import { pointForRelayTransport } from "../../../../web/simulator/android/orientation";
...
describe("Android rotated emulator input", () => {
	test("derives logical dimensions from the device's native aspect", () => {
		...
		expect(logicalSizeForRotation(phone, 1)).toEqual({
			width: 2424,
			height: 1080,
		});
		expect(logicalSizeForRotation(phone, 2)).toEqual(phone);
		expect(logicalSizeForRotation(phone, 3)).toEqual({
			width: 2424,
			height: 1080,
		});

		expect(logicalSizeForRotation(tablet, 0)).toEqual(tablet);
		expect(logicalSizeForRotation(tablet, 1)).toEqual({
			width: 1600,
			height: 2560,
		});
		expect(logicalSizeForRotation(tablet, 2)).toEqual(tablet);
		expect(logicalSizeForRotation(tablet, 3)).toEqual({
			width: 1600,
			height: 2560,
		});
	});
	...
	test("keeps the toolbar orientation cycle correct for landscape-native tablets", () => {
		...
		for (const rotation of [0, 1, 2, 3] as const) {
			const logical = logicalSizeForRotation(
				{ width: 2560, height: 1600 },
				rotation,
			);
			expect(androidOrientationForScreen({ ...logical, rotation })).toBe(
				expected[rotation],
			);
			expect(
	...
	test("keeps the standard orientation cycle for portrait-native phones", () => {
		...
		for (const rotation of [0, 1, 2, 3] as const) {
			...
			expect(
				androidRotationForOrientation(expected[rotation], {
					...logical,
					rotation,
				}),
			).toBe(rotation);
	...
	test("wakes only a non-interactive emulator before rotating", () => {
		expect(
			androidPowerNeedsWake("mWakefulness=Awake\nmIsInteractive=true"),
		).toBe(false);
		expect(
			androidPowerNeedsWake("mWakefulness=Asleep\nmIsInteractive=false"),
		).toBe(true);
