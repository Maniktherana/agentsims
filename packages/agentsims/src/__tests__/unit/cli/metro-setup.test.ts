...
describe("transformMetroConfig", () => {
	test("wraps a canonical Expo CommonJS config and is byte-idempotent", () => {
		...
		expect(
			transformMetroConfig(transformed.source, "/app/metro.config.js"),
		).toEqual({
	...
	test("supports Sentry's resolved Expo config factory", () => {
		...
		expect(
			transformMetroConfig(source, "/app/metro.config.js").source,
	...
	test("reuses aliased Agentsims imports and completes partial imports", () => {
		...
		expect(
			transformMetroConfig(aliased, "/app/metro.config.mjs").source,
		).toContain("export default configureAgentsims(config);");
	...
	test("refuses configs that cannot provide a resolved Babel transformer", () => {
		...
		expect(() =>
			transformMetroConfig(asyncMerge, "/app/metro.config.js"),
		).toThrow("Async configs");
		...
		expect(() =>
			transformMetroConfig(reassigned, "/app/metro.config.js"),
		).toThrow("not a supported resolved Metro config");
	});
	...
	test("refuses inner wrappers and explicit instrumentation opt-out", () => {
		...
		expect(() =>
			transformMetroConfig(disabled, "/app/metro.config.js"),
		).toThrow("instrumentBabel: false");
	});
});

describe("Metro setup planning and writes", () => {
	test("creates canonical Expo and bare React Native plans", () => {
		const expo = temporaryProject({
			expo: "*",
			"react-native": "*",
			agentsims: "*",
		});
		...
		const reactNativePlan = planMetroSetup(
			{ project: reactNative.root },
			fixedSystem,
		);
		expect(reactNativePlan.framework).toBe("react-native");
		expect(reactNativePlan.updated).toContain(
	...
	test("follows Expo's canonical tsx loader to its TypeScript config", () => {
		...
		expect(readFileSync(project.configPath, "utf8")).not.toContain(
			"withAgentsims",
		);
	});
	...
	test("aborts if the config changes after the diff was planned", () => {
		...
		expect(() => applyMetroSetup(plan, fixedSystem)).toThrow(
			"changed after setup was planned",
		);
		expect(readFileSync(project.configPath, "utf8")).toEndWith(
			"// concurrent edit\n",
		);
		expect(
			existsSync(`${project.configPath}.agentsims.bak.20260806T123456Z`),
		).toBe(false);
	});
	...
	test("does not overwrite a backup created concurrently", () => {
		...
		expect(applied.backupPath).toBe(
			`${project.configPath}.agentsims.bak.20260806T123456Z.2`,
		);
		expect(
			readFileSync(
				`${project.configPath}.agentsims.bak.20260806T123456Z`,
...
describe("runSetupCommand", () => {
	test("accepts a project path as the setup argument", () => {
		...
		expect(() =>
			setupOptionsForProjectPath("/app", { project: "/other" }),
		).toThrow("either the setup project path or --project");
	});
	...
	test("requires --yes without a TTY and applies with it", async () => {
		...
		expect(readFileSync(project.configPath, "utf8")).toContain(
			"withAgentsims(config",
		);
		expect(output.stdout()).toContain(
			"Restart Metro to activate source mapping.",
		);
	});
});
