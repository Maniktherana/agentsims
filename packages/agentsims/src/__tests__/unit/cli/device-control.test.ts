...
describe("device control HTTP client", () => {
	test("gets one observation from the server and writes its screenshot", async () => {
		...
		try {
			...
			expect(requests).toEqual(["GET /device/android%3Aemulator-5554/observe"]);
		} finally {
			server.stop(true);
...
describe("parseAgentAction", () => {
	test("parses every supported cross-platform action", () => {
		...
		expect(
			parseAgentAction('{"type":"swipe","x1":0.5,"y1":0.8,"x2":0.5,"y2":0.2}'),
		).toEqual({
			type: "swipe",
			x1: 0.5,
			y1: 0.8,
		...
		expect(
			parseAgentAction('{"type":"rotate","orientation":"landscape_left"}'),
		).toEqual({
			type: "rotate",
	...
	test("rejects invalid or unbounded actions", () => {
		...
		expect(() =>
			parseAgentAction('{"type":"rotate","orientation":"upside-down"}'),
		).toThrow("orientation");
		expect(() =>
			parseAgentAction('{"type":"launch","app":"Settings"}'),
