...
describe("collectAndroidAxSnapshot", () => {
	...
	test("keeps screen geometry physical and clamps hidden off-screen bounds", async () => {
		...
		expect(result.screen).toEqual({ width: 1080, height: 2424 });
		expect(
			result.elements.find((element) => element.label === "Off-screen content")
				?.frame,
		).toEqual({ x: 0, y: 2100, width: 1080, height: 324 });
		expect(
			result.elements.find((element) => element.windowId === 11)?.frame,
	...
	test("keeps the base app tree and stable paths while a sheet window is open", async () => {
		...
		const read = (xml: string) =>
			collectAndroidAxSnapshot("emulator-5554", {
		...
		expect(open.elements.map((element) => element.label)).toContain(
			"Ask Vartalaap",
		);
		expect(open.elements.map((element) => element.label)).toContain(
			"Close sheet",
		...
		expect(
			open.elements.find((element) => element.label === "Ask Vartalaap")?.path,
		).toBe("0.0");
		expect(
			closed.elements.find((element) => element.label === "Ask Vartalaap")
				?.path,
		).toBe("0.0");
		expect(
			open.elements.find((element) => element.label === "Close sheet")?.path,
		).toBe("1.0");
	});
