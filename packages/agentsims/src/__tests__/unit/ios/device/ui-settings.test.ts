...
describe("parseUiArgs", () => {
	...
	test("dangling -d/--device flag is an error", () => {
		expect(parseUiArgs(["appearance", "-d"]).error).toMatch(
			/requires a value/i,
		);
		expect(parseUiArgs(["status", "--device"]).error).toMatch(
			/requires a value/i,
		);
	});
	...
	test("color filter accepts vision-deficiency aliases", () => {
		...
		expect(parseUiArgs(["color-filter", "deuteranopia"]).value).toBe(
			"green-red",
		);
		expect(parseUiArgs(["color-filter", "tritanopia"]).value).toBe(
			"blue-yellow",
		);
