...
describe("Android presented surface geometry parity", () => {
  test("matches the devtool retained display and relative r0-r3 contract", () => {
    expect(retainedAndroidDisplayOrientation(null, "portrait_upside_down", "portrait"))
      .toBe("portrait_upside_down");
    expect(retainedAndroidDisplayOrientation("portrait_upside_down", null, "portrait"))
      .toBe("portrait_upside_down");
    expect([
      "portrait",
      "landscape_left",
      ...
      "landscape_right",
    ].map((displayed) => relativeAndroidOrientation(
      displayed as "portrait" | "landscape_left" | "portrait_upside_down" | "landscape_right",
      "portrait",
    ))).toEqual([
      "portrait",
      "landscape_left",
    ...
    expect(relativeAndroidPlaneStyle(
  ...
  test("keeps exactly one visual transform owner around untransformed media and AX", async () => {
    const simulatorView = await Bun.file(new URL(
      "../web/simulator/SimulatorView.tsx",
      import.meta.url,
    )).text();
    const workspace = await Bun.file(new URL(
      "../web/workspace/simulator-device-view.tsx",
      import.meta.url,
    )).text();
    ...
    expect(simulatorView).toContain('position: hasPresentationPlane ? "absolute" : "relative"');
    expect(simulatorView).toContain('inset: hasPresentationPlane ? 0 : undefined');
    expect(simulatorView).toContain(
      '? (presentationRotationDegrees ?? 0)',
    );
    expect(workspace).toContain('presentationPlaneStyle={isAndroidDevice ? effectivePlane.planeStyle : undefined}');
