...
import { proxyPreviewConfigForBrowser } from "../web/utils/preview-config";
...
describe("proxyPreviewConfigForBrowser", () => {
  ...
  test("advertises AVCC as the only Android stream transport", () => {
    expect(proxyPreviewConfigForBrowser(
      {
        ...baseConfig,
      ...
      "http://localhost:3210/helper/android%3Aemulator-5554/stream.avcc",
    );
  });
});
