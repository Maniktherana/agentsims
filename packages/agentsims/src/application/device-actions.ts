...
import { textToKeyEvents, UnsupportedCharacterError } from "../shared/text-to-keys";
...
function stepsForAction(action: DeviceAction): InputStep[] {
  switch (action.type) {
    case "tap":
      return [
        {
          data: inputFrame(INPUT_TOUCH, { type: "begin", x: action.x, y: action.y }),
          delayAfterMs: 40,
        },
        { data: inputFrame(INPUT_TOUCH, { type: "end", x: action.x, y: action.y }) },
      ];
    case "gesture":
      return [{
    ...
    case "swipe": {
      ...
      return [
        {
          data: inputFrame(INPUT_TOUCH, { type: "begin", x: action.x1, y: action.y1 }),
          delayAfterMs,
        },
        {
          data: inputFrame(INPUT_TOUCH, { type: "move", x: action.x2, y: action.y2 }),
          delayAfterMs,
        },
        { data: inputFrame(INPUT_TOUCH, { type: "end", x: action.x2, y: action.y2 }) },
      ];
    }
    case "type":
      try {
    ...
    case "rotate":
      return [{ data: inputFrame(INPUT_ROTATE, { orientation: action.orientation }) }];
  }
}

async function defaultResolveSession(device: string): Promise<DeviceInputSession> {
...
export function decodeDeviceAction(value: unknown): DeviceAction {
  ...
  switch (action.type) {
    case "tap":
      return { type: "tap", x: normalized(action.x, "x"), y: normalized(action.y, "y") };
    case "gesture": {
      if (
        action.phase !== "begin" &&
        action.phase !== "move" &&
    ...
    case "rotate":
      if (typeof action.orientation !== "string" || !VALID_ORIENTATIONS[action.orientation]) {
        throw new Error(`orientation must be one of ${Object.keys(VALID_ORIENTATIONS).join(", ")}`);
      }
...
export function parseDeviceAction(value: string): DeviceAction {
  try {
    ...
    if (error instanceof SyntaxError) throw new Error("Action must be valid JSON");
    throw error;
  }
}
...
export class DeviceActionCommands {
  ...
  async act(device: string, values: ReadonlyArray<unknown>): Promise<void> {
    if (!device) throw new Error("Invalid or missing device");
    if (values.length === 0) throw new Error("At least one action is required");
    const actions = values.map(decodeDeviceAction);
    const session = await this.resolveSession(device);
    for (const action of actions) {
      for (const step of stepsForAction(action)) {
        await session.dispatchInputFrame(step.data);
        if (step.delayAfterMs) await this.pause(step.delayAfterMs);
      }
    }
  }
}
