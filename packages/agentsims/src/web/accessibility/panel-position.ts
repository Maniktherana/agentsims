...
export function parseAccessibilityPanelGeometry(
	...
	try {
		const parsed = JSON.parse(
			value ?? "null",
		) as Partial<SavedAccessibilityPanelGeometry> | null;
		if (
			!parsed ||
...
): AccessibilityPanelAnchorRect | null {
...
export function resetAccessibilityPanelGeometryForRect(
	...
	return defaultAccessibilityPanelGeometryForRect(
		rect,
		viewportWidth,
		viewportHeight,
	);
...
export function useAccessibilityPanelPosition(
	...
	const apply = useCallback(
		(geometry: AccessibilityPanelGeometry, persist = false) => {
	...
	const onResizePointerDown = useCallback(
		(event: ReactPointerEvent<HTMLDivElement>) => {
			...
			const move = (next: PointerEvent) =>
				apply(
					resizeAccessibilityPanelGeometryFromPointer(
						start,
						startPoint,
	...
	const onResizeKeyDown = useCallback(
		(event: ReactKeyboardEvent<HTMLDivElement>) => {
			const delta = accessibilityPanelResizeDeltaForKey(
				event.key,
				event.shiftKey,
			);
			if (!delta) return;
