...
export function axNodeForElement(element: AxElement, index: number) {
	...
	const sourceLabel =
		element.source?.componentName || element.source?.elementName;
...
export function meaningfulAxTargetElements(
	...
	const useful = elements.filter((element) => {
		...
		const sourceName =
			`${element.source?.elementName ?? ""} ${element.source?.componentName ?? ""}`.toLowerCase();
		...
		if (areaRatio > 0.35 && isContainerRole(element) && !meaningfulRole)
			return false;
		if (
			!human &&
			...
			return false;
		if (areaRatio > 0.35 && !human && !meaningfulRole && !meaningfulSource)
			return false;
