...
import { ScreenshotToast } from "../../components/feedback/screenshot-toast";
...
export function useScreenshotToast(deviceUdid?: string | null) {
	...
	const pauseDismiss = useCallback(() => {
		...
		remainingDismissMsRef.current = Math.max(
			0,
			dismissDeadlineRef.current - Date.now(),
		);
		clearDismissTimer();
	}, [clearDismissTimer]);
	...
	const render = useCallback(
		(next: ScreenshotToast, duration = Infinity) => {
	...
	const capture = useCallback(async () => {
		...
		const file = `$HOME/Desktop/agentsims-screenshot-${timestampSlug()}.png`;
		const capCmd = `F="${file}"; xcrun simctl io ${shellEscape(deviceUdid)} screenshot "$F" && printf '%s' "$F"`;
				...
				render(
					{
						...
						status: "error",
						phase: "in",
						message: res.stderr.trim() || "Screenshot failed",
					},
					ERROR_DISMISS_MS,
				);
				return;
		...
		const thumb = `/tmp/agentsims-screenshot-thumb-${id}.png`;
		try {
			...
			if (b64) {
				...
				if (current?.id === id) {
					render(
						{ ...current, thumb: `data:image/png;base64,${b64}` },
						SAVED_DISMISS_MS,
					);
				}
			}
