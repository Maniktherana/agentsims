...
export function useSimulatorResize({
	...
	const recordSample = useCallback((x: number) => {
		...
		while (
			buf.length > 1 &&
			t - buf[0]!.t > SIMULATOR_RESIZE_VELOCITY_HISTORY_MS
		)
			buf.shift();
	}, []);
	...
	const endDrag = useCallback(() => {
		...
		const trivial =
			Math.abs(fromDisplay - target) < 0.5 && Math.abs(velocity) < 8;
	...
	useEffect(() => {
		...
		const updateFromPointer = (event: PointerEvent) => {
			...
			scheduleMove(
				widthFromDelta(s, event.clientX, event.clientY, aspectRatio),
			);
		};
