...
export type WorkspaceSelectionAction =
	...
	| {
			...
			devices: readonly WorkspaceGridDevicePresence[];
	  }
	| {
			type: "device-started";
...
function reconcileRunningDevices(
	...
	const hiddenRunningDeviceIds = new Set(
		[...state.hiddenRunningDeviceIds].filter((deviceId) =>
			running.has(deviceId),
		),
	);
...
export function workspaceSelectionReducer(
	...
	switch (action.type) {
		...
		case "set-visible": {
			...
			const next = updateSelectionSets(
				state,
				visibleDeviceIds,
		...
		case "device-started": {
			...
			return {
				selectedDeviceId: action.focus
					? action.resolvedDeviceId
...
export function visibleRunningDeviceIds(
	...
	return runningDeviceIds.filter((deviceId) =>
		selection.visibleDeviceIds.has(deviceId),
	);
}
