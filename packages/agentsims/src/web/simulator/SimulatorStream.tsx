...
export interface SimulatorStreamProps {
  /** Gateway exec function for running agentsims CLI commands. */
  ...
  /** Called with the active agentsims device UDID (or null when not streaming). */
  onActiveDeviceChange?: (udid: string | null) => void;
}

/**
 * Full agentsims panel: connect/disconnect lifecycle + stream viewer.
 * Uses the gateway exec to invoke the `agentsims` CLI on the host,
 * then connects directly to the agentsims server for video + touch.
 */
export function SimulatorStream({ exec, device, style, imageStyle, className, stream, headerless, onStreamingChange, onScreenConfigChange, onError, onActiveDeviceChange, enableDigitalCrown, codec }: SimulatorStreamProps) {
  ...
  // Bubble the active agentsims device UDID to parent.
  useEffect(() => {
          ...
          <div style={{ display: "flex", gap: 6 }}>
            ...
            {info ? (
              ...
              <button onClick={async () => {
                console.log(`[agentsims] Connect clicked`);
                const ok = await connect();
                console.log(`[agentsims] connect() resolved (ok=${ok})`);
                if (ok && relayMode) {
                  console.log(`[agentsims] sending stream:start`);
                  stream.start({ maxFps: 30 });
                }
