...
/** Matches the JSON output of `agentsims --detach` and `agentsims --list`. */
...
export interface UseSimStreamResult {
  /** Connection info when a agentsims server is running, null otherwise. */
  ...
  /** Stop the running agentsims server. */
...
export function useSimStream({ exec, device: deviceProp }: UseSimStreamOptions): UseSimStreamResult {
  ...
  // `agentsims --detach` handles booting, tearing down a previous server
    ...
    (async () => {
      ...
      try {
        if (!deviceProp) {
          ...
          await exec("agentsims --kill");
          if (mountedRef.current) setInfo(null);
          return;
        }
        const result = await exec(`agentsims --detach ${deviceProp}`);
        if (cancelled) return;
        if (result.exitCode !== 0) {
          throw new Error(result.stderr || `agentsims --detach failed (exit ${result.exitCode})`);
        }
  ...
  const connect = useCallback(async (device?: string, port?: number): Promise<boolean> => {
    ...
    console.log(`[agentsims] connect: starting`);
    try {
      const target = device ?? deviceProp ?? undefined;
      let cmd = "agentsims --detach";
      if (target) cmd += ` ${target}`;
      if (port) cmd += ` --port ${port}`;

      const result = await exec(cmd);
      console.log(`[agentsims] connect: exec returned (+${(performance.now() - t0).toFixed(0)}ms, exit ${result.exitCode})`);
      if (result.exitCode !== 0) {
        throw new Error(result.stderr || `agentsims --detach failed (exit ${result.exitCode})`);
      }
      const parsed = JSON.parse(result.stdout.trim()) as SimStreamInfo;
  ...
  const sendButton = useCallback(async (button: string) => {
    try {
      await exec(`agentsims button ${button}`);
    } catch {
      // best-effort
    }
  }, [exec]);
