...
export function relativeAndroidPlaneStyle(
	...
	const displayRatio = displayConfig
...
export function androidCutoutEdge(
	...
	return ["top", "right", "bottom", "left"][
		rotation
	] as AndroidPresentation["cutoutEdge"];
}
...
export function androidPresentation(
	...
	const sideways =
		geometry.needsCssRotation && Math.abs(geometry.rotationDegrees) === 90;
	const displayRatio = displayConfig
		? displayConfig.width / displayConfig.height
		: 1;
	return {
		...geometry,
		displayConfig,
		cutoutEdge: androidCutoutEdge(
			Math.max(0, ORIENTATIONS.indexOf(config?.orientation ?? "portrait")) as
				| 0
				| 1
				| 2
				| 3,
		...
		planeStyle: {
			...
			transform: `translate(-50%, -50%)${
				geometry.rotationDegrees === 0
					? ""
					: ` rotate(${geometry.rotationDegrees}deg)`
			}`,
		},
	};
}
