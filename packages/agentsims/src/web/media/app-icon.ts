import { type ExecResult, shellEscape } from "../simulator/input/exec";
...
export async function fetchAppDetails(
	...
	const ctn = await exec(
		`xcrun simctl get_app_container ${udid} ${shellEscape(bundleId)} app`,
	);
	if (ctn.exitCode !== 0) {
	...
	const plist = await exec(
		`plutil -convert json -o - ${shellEscape(appPath + "/Info.plist")}`,
	);
	let info: any = {};
	if (plist.exitCode === 0) {
		try {
	...
	const iconFiles: string[] | undefined =
		primary?.CFBundleIconFiles ?? info?.CFBundleIconFiles;
	if (iconFiles && iconFiles.length > 0)
		iconName = iconFiles[iconFiles.length - 1];
	else if (typeof info?.CFBundleIconFile === "string")
