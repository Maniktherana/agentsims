...
export const DROP_HOST_PATH_TYPE = "application/x-agentsims-host-path";

function androidSerial(deviceId: string): string | null {
	return deviceId.startsWith("android:")
		? deviceId.slice("android:".length)
		: null;
}

function androidMediaName(path: string): string {
	const name = path.split("/").pop() ?? "agentsims-media";
	return name.replace(/[^A-Za-z0-9._-]/g, "_");
}

async function addHostMediaToAndroid(
	path: string,
	exec: (command: string) => Promise<ExecResult>,
	serial: string,
): Promise<void> {
	const remotePath = `/sdcard/Pictures/${androidMediaName(path)}`;
	const push = await exec(
		`adb -s ${shellEscape(serial)} push ${shellEscape(path)} ${shellEscape(remotePath)}`,
	);
	if (push.exitCode !== 0) {
		throw new Error(push.stderr || `adb push failed (exit ${push.exitCode})`);
	}
	const scan = await exec(
		`adb -s ${shellEscape(serial)} shell am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE -d ${shellEscape(`file://${remotePath}`)}`,
	);
	if (scan.exitCode !== 0) {
		throw new Error(scan.stderr || `media scan failed (exit ${scan.exitCode})`);
	}
}
...
export async function addHostMediaToPhotos(
	...
	const serial = androidSerial(udid);
	if (serial) return addHostMediaToAndroid(path, exec, serial);
	const result = await exec(
		`xcrun simctl addmedia ${udid} ${shellEscape(path)}`,
	);
	if (result.exitCode !== 0) {
		throw new Error(
			result.stderr || `addmedia failed (exit ${result.exitCode})`,
		);
	}
}
...
async function streamFileToHostPath(
	...
	for (let offset = 0; offset < file.size; offset += DROP_CHUNK_BYTES) {
		const slice = await file
			.slice(offset, offset + DROP_CHUNK_BYTES)
			.arrayBuffer();
		const chunk = arrayBufferToBase64(slice);
		const op = offset === 0 ? ">" : ">>";
		const result = await exec(
			`bash -c 'echo ${chunk} | base64 -d ${op} ${tmpPath}'`,
		);
		if (result.exitCode !== 0) {
			throw new Error(
				result.stderr || `Write failed (exit ${result.exitCode})`,
			);
		}
...
// panel to stage image/video sources for `agentsims camera --file`.
...
export async function uploadDroppedFile(
	...
	const prefix = kind === "ipa" ? "agentsims-install" : "agentsims-upload";
	const tmpPath = `/tmp/${prefix}-${crypto.randomUUID()}.${ext}`;
	...
	try {
		...
		if (serial) {
			if (kind === "ipa")
				throw new Error("IPA files can only be installed on iOS simulators");
			await addHostMediaToAndroid(tmpPath, exec, serial);
		} else {
			const cmd =
				kind === "ipa"
			...
			if (result.exitCode !== 0) {
				...
				throw new Error(
					result.stderr || `${label} failed (exit ${result.exitCode})`,
				);
			}
		}
	} finally {
