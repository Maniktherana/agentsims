...
export function appendSimulatorFrameRateHistory(
  ...
  return [...history.filter((point) => point.time >= cutoff), { time, value }].slice(
    -HISTORY_MAX_POINTS,
  );
}
...
export class SimulatorFrameRateStore {
  ...
  readonly getHistorySnapshot = (): SimulatorFrameRatePoint[] => this.historySnapshot;
  readonly getServerHistorySnapshot = (): SimulatorFrameRatePoint[] => this.historySnapshot;
  ...
  recordTiming(sequence: bigint, timestampUs: bigint): void {
    ...
    if (this.samples.length >= 2) {
      ...
      if (elapsedUs > 0n && producedFrames > 0n) {
        const rounded = (producedFrames * 1_000_000n + elapsedUs / 2n) / elapsedUs;
