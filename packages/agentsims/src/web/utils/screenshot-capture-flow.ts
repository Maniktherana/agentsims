import type { CapturedScreenshotPreview } from "../hooks/use-device-screenshot-feedback";
...
export function startScreenshotCapture({
  ...
  replace: (requestId: number, capture: CapturedScreenshotPreview) => boolean | void;
  ...
  try {
    ...
    reportError(
      error instanceof Error ? error.message : "Unable to capture rendered screen",
    );
    return null;
  }
  if (!optimistic) {
