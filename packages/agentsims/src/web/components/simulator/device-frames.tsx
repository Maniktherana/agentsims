...
export function simulatorAspectRatio(
	...
	const size =
		config && config.width > 0 && config.height > 0 ? config : fallback;
...
export function screenBorderRadius(
	...
	config?: Pick<
		StreamConfig,
		"width" | "height" | "orientation" | "cornerRadii"
	> | null,
) {
...
export function simulatorResizeCornerArc(params: {
	...
	const gutter = Math.max(
		2.2,
		Math.min(5.2, 0.011 * Math.min(containerWidth, containerHeight)),
	);
