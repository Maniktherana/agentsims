...
export function getDeviceType(name?: string | null): DeviceType {
  ...
  if (lower.includes("android") || lower.includes("pixel") || lower.includes("gphone") || lower.includes("sdk")) return "android";
  return "iphone";
}
...
function AndroidFrameChrome({ streaming = false }: { streaming?: boolean }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: '100%', height: '100%' }} fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="0.833" y="0.833" width={w - 1.666} height={h - 1.666} rx={outerRx} stroke="#35363a" strokeWidth="1.666" />
      <g opacity="0.9" filter="url(#android-glow0)">
        <rect x="2" y="2" width={w - 4} height={h - 4} rx={outerRx - 2} stroke="#4b4d52" strokeWidth="1.5" />
      </g>
      <rect x="7" y="7" width={w - 14} height={h - 14} rx={innerRx} stroke="#696b70" strokeWidth="0.8" opacity="0.75" />
      <circle
        cx={w / 2}
        cy={17}
        r="3.2"
        fill="#111"
        style={{ opacity: streaming ? 0 : 0.9, transition: 'opacity 0.3s ease' }}
      />
      <rect
        x={(w - 92) / 2}
        y={h - 18}
        ...
        rx="2"
        fill="white"
        style={{ opacity: streaming ? 0 : 0.3, mixBlendMode: 'difference', transition: 'opacity 0.3s ease' }}
      />
    </svg>
  );
}
