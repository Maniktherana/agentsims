...
function useToolbar(component: string): ToolbarContextValue {
	...
	if (!ctx) {
		throw new Error(
			`<SimulatorToolbar.${component}> must be rendered inside <SimulatorToolbar>`,
		);
	}
	return ctx;
}
...
export interface SimulatorToolbarProps extends HTMLAttributes<HTMLDivElement> {
	exec: ExecFn;
	/** Optional direct rotate handler. Defaults to shelling out to `agentsims rotate`. */
	onRotate?: RotateFn;
...
//
// The HID home-button press (`agentsims button home`) is silently dropped by
...
export function homeButtonCommand(
	...
	if (deviceUdid)
		return `xcrun simctl launch ${deviceUdid} com.apple.springboard`;
	return "agentsims button home";
}
...
const HomeButton = forwardRef<HTMLButtonElement, ToolbarButtonProps>(
	function HomeButton({ onClick, ...rest }, ref) {
		...
		return (
			<ToolbarButton
				...
				onClick={(e) => {
					...
					if (ctx.deviceUdid) {
						void ctx.exec(
							`xcrun simctl io ${ctx.deviceUdid} screenshot ~/Desktop/agentsims-screenshot-$(date +%s).png`,
						);
					}
				}}
				{...rest}
...
const RotateButton = forwardRef<HTMLButtonElement, ToolbarButtonProps>(
	function RotateButton({ onClick, forceDisabled, ...rest }, ref) {
		const ctx = useToolbar("RotateButton");
		const cantRotate =
			ctx.deviceType === "watch" || ctx.deviceType === "vision";
		// Reset the cycle when the device changes — each sim boots in portrait.
		const [orientation, setOrientation] =
			useState<SimulatorOrientation>("portrait");
		useEffect(() => {
			...
			<ToolbarButton
				...
				onClick={(e) => {
					...
					if (ctx.onRotate) {
						...
						void ctx.exec(`agentsims rotate ${next} -d ${ctx.deviceUdid}`);
					}
				}}
				{...rest}
			>
...
export const SimulatorToolbar =
	SimulatorToolbarRoot as SimulatorToolbarCompound;
