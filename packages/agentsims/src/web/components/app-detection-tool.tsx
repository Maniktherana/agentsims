  ...
  return (
    ...
    >
      ...
      <dl className="m-0 flex flex-col gap-1.5">
            {!isAndroid && <Row label="Version" value={details.shortVersion ? `${details.shortVersion} (${details.bundleVersion ?? "—"})` : details.loading ? "…" : "—"} />}
            {!isAndroid && <Row label="Min iOS" value={details.minOS ?? (details.loading ? "…" : "—")} />}
            {!isAndroid && <Row label="Executable" value={details.executable ?? (details.loading ? "…" : "—")} />}
            <Row label="PID" value={details.pid != null ? String(details.pid) : "—"} />
            <Row label="React Native" value={details.isReactNative ? "Yes" : "No"} />
            {!isAndroid && <Row
              label="App path"
