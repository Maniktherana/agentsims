...
import type { ScreenshotToast as ScreenshotToastState } from "../../hooks/feedback/use-screenshot-toast";
import { DROP_HOST_PATH_TYPE } from "../../media/drop";
...
export function shouldDismissScreenshotToastAfterDrag(
...
export function ScreenshotToast({
	...
	const dragFallbackTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	...
	const handleDragEnd = (e: DragEvent<HTMLButtonElement>) => {
		clearDragFallback();
		setDragging(false);
		if (shouldDismissScreenshotToastAfterDrag(e.dataTransfer.dropEffect))
			onDismiss();
		else onResume();
	};
			...
			{toast.status === "error" ? (
				<div className="flex items-center gap-2 px-3.5 py-2.5 bg-panel border border-white/12 rounded-xl text-white/90 text-[12px] shadow-[0_8px_24px_rgba(0,0,0,0.45)]">
					...
					<span className="select-text">
						{toast.message ?? "Screenshot failed"}
					</span>
				</div>
			) : (
				<button
