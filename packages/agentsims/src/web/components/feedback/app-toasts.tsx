...
import type { UploadToast } from "../../hooks/feedback/use-upload-toasts";
			...
			<div className="flex items-center gap-2 min-w-0">
				...
				<span className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
					...
					{isUploading &&
						!transferring &&
						(toast.kind === "ipa"
							? `Installing ${toast.name}…`
							: `Adding ${toast.name}…`)}
					{toast.status === "success" &&
						(toast.kind === "ipa"
							? `Installed ${toast.name}`
							: `Added ${toast.name} to Photos`)}
			...
			{isUploading && (
				<div className="relative h-[3px] w-full bg-white/8 rounded-[2px] overflow-hidden">
					{transferring ? (
						...
						<div className="agentsims-toast-indeterminate absolute top-0 left-0 h-full w-[40%] bg-accent rounded-[2px]" />
					)}
				</div>
			)}
		</div>
