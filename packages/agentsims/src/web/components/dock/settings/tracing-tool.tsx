...
export function TracingTool({
	...
	return (
		...
		>
			<SettingRow icon={<Route size={14} strokeWidth={2} />} label="Enable tracing">
				<SettingSwitch
			...
			{detail ? (
				<div className="flex min-w-0 items-center gap-1.5 ps-7 pt-1">
					<div className="flex min-w-0 flex-1 items-center gap-2 text-[12px] tabular-nums">
						<span
							className="min-w-0 truncate text-white/62"
							title={detail.id}
						>
							{summary?.name ?? (live ? "Current trace" : "Latest trace")}
						</span>
						<span className="shrink-0 text-white/42">{detail.calls.length} calls</span>
						{refused > 0 && (
						...
						<span className="shrink-0 text-white/42">{formatElapsed(spanMs)}</span>
					</div>
