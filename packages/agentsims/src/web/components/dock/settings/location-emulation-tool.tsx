...
export function LocationEmulationTool({
	...
	useEffect(() => {
		...
		if (previous && previous !== prepared) {
			morphRef.current = buildMorphState(
				previous,
				previousArc,
				prepared,
				performance.now(),
	...
	useEffect(() => {
		...
		const tick = (ts: number) => {
			...
			cameraAngleRef.current =
				(cameraAngleRef.current + dt * 0.00012) % (Math.PI * 2);
				...
				if (morph) {
					...
					if (t >= 1) {
						...
						markerOverride = {
							x:
								morph.prevMarker.x +
								(morph.nextMarker.x - morph.prevMarker.x) * eased,
							y:
								morph.prevMarker.y +
								(morph.nextMarker.y - morph.prevMarker.y) * eased,
							z:
								morph.prevMarker.z +
								(morph.nextMarker.z - morph.prevMarker.z) * eased,
						};
					}
				}
				renderScene(
	...
	const onPlayPause = useCallback(() => {
		...
		if (
			arcRef.current >= trailRef.current.totalDistance &&
			!trailRef.current.trail.loop
		) {
			arcRef.current = 0;
			elapsedRef.current = 0;
		}
...
function buildMorphState(
	...
	const toPlanar = Math.max(
		1,
		to.bounds.x[1] - to.bounds.x[0],
		to.bounds.z[1] - to.bounds.z[0],
	);
...
function nearestNeighbourAlign(
	from: Vec3[],
	to: Vec3[],
	loop: boolean,
): Vec3[] {
	...
	const out: Vec3[] = [];
	for (let i = 0; i < N; i++) out.push(target[(i + bestOffset) % N]!);
	return out;
}
...
function morphFrame(state: MorphState, t: number): PreparedTrail {
	...
	return {
		...
		rawMinAlt:
			state.to.rawMinAlt +
			(state.from.rawMinAlt - state.to.rawMinAlt) * (1 - t),
		rawMaxAlt:
			state.to.rawMaxAlt +
			(state.from.rawMaxAlt - state.to.rawMaxAlt) * (1 - t),
	};
}
...
function renderScene(
	...
	const scale =
		Math.min((cssW - padX * 2) / fitW, (cssH - padY * 2) / fitH) * 1.45;
	...
	const elevationGain =
		yExtent < 1 ? 1 : Math.min(24, 140 / Math.max(20, yExtent));
