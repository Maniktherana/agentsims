...
import { execOnHost, shellEscape, type ExecResult } from "../../../simulator/input/exec";
...
export function parseAndroidSimulatorSettings(input: {
  ...
  return {
    appearance: /\bNight mode:\s*yes\b/i.test(input.nightMode) ? "dark" : "light",
...
function commandValue(result: ExecResult, fallback: string): string {
  if (result.exitCode !== 0) {
    throw new Error(result.stderr.trim() || `adb exited with ${result.exitCode}`);
  }
...
export function AndroidSimulatorSettingsTool({ udid }: { udid: string }) {
  const serial = udid.startsWith("android:") ? udid.slice("android:".length) : udid;
  const escapedSerial = shellEscape(serial);
  const [open, setOpen] = useState(true);
  const [settings, setSettings] = useState<AndroidSimulatorSettings | null>(null);
  ...
  const refresh = useCallback(async (signal?: AbortSignal) => {
    setError(null);
    try {
      const [nightMode, fontScale, animationScale, showTouches, pointerLocation] =
        await Promise.all([
      ...
      setSettings(parseAndroidSimulatorSettings({
  ...
  const run = useCallback(async (
    ...
    setSettings(next);
    try {
      const results = await Promise.all(commands.map((command) => adb(command)));
  ...
  return (
    <CollapsibleSection
      ...
      summary={<span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-white/50">Simulator</span>}
    ...
    >
      ...
      <SettingRow icon={<Icon><MoonStar size={14} /></Icon>} label="Appearance">
        <SettingSelect
          label="Appearance"
      ...
      <SettingRow icon={<Icon><Type size={14} /></Icon>} label="Text Size">
        <TextSizeSlider
      ...
      <SettingRow icon={<Icon><Gauge size={14} /></Icon>} label="Reduce Motion">
        <SettingSwitch
          label="Reduce Motion"
      ...
      <SettingRow icon={<Icon><CircleDot size={14} /></Icon>} label="Show Touches">
        <SettingSwitch
          label="Show Touches"
          checked={shown.showTouches}
          disabled={!ready || pending === "show-touches"}
          onChange={(showTouches) => void run(
            "show-touches",
            { ...shown, showTouches },
      ...
      <SettingRow icon={<Icon><MousePointerClick size={14} /></Icon>} label="Pointer Location">
        <SettingSwitch
          label="Pointer Location"
          checked={shown.pointerLocation}
          disabled={!ready || pending === "pointer-location"}
          onChange={(pointerLocation) => void run(
            "pointer-location",
