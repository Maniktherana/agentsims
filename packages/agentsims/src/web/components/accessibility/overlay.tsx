...
function compareDuplicateTargetPreference(
	...
	const rankDifference =
		actionableSemanticRank(left.element) -
		actionableSemanticRank(right.element);
...
function collapseDuplicateAxOverlayTargets(
	...
	for (
		...
		while (ancestorPath) {
			...
			const ancestor =
				ancestorIndex === undefined ? null : entries[ancestorIndex]!;
			if (
				ancestor &&
...
export function buildAxOverlayTargetEntries(
	...
	const projectedTargetByCandidatePath = new Map(
		undeduplicatedEntries.flatMap((entry) => {
			const retainedKey = collapsedTargets.retainedKeyByCandidateKey.get(
				entry.key,
			);
			const retainedEntry = retainedKey ? eligibleByKey.get(retainedKey) : null;
			return retainedEntry
				? [[entry.element.path, retainedEntry] as const]
				: [];
		}),
	);
	...
	for (const entry of visibleEntries) {
		...
		while (path) {
			const target =
				projectedTargetByCandidatePath.get(path) ?? eligibleByPath.get(path);
			if (target) {
...
export function projectAxOverlayTargetKeys(
	...
	return new Set([...rawKeys].map((key) => previewKeyByRawKey.get(key) ?? key));
}
...
export function AxDomOverlay({
	...
	const handlePhoneHighlight = useCallback(
		(key: string | null) => {
	...
	const highlightedFrame = highlightedElement
		? clampAxFrameForScreen(highlightedElement.frame, {
				width: screenWidth,
				height: screenHeight,
			})
		: null;
	const hover =
		highlightedElement && shouldShowAxPhoneTooltip(mode, highlightedOrigin)
	...
	const [overlayRect, setOverlayRect] = useState<OverlayViewportRect | null>(
		null,
	);
	...
	const hoverPosition = useMemo<HoverPosition | null>(() => {
		...
		const targetBottom =
			overlayRect.top + (highlightedFrame.y + highlightedFrame.height) * scaleY;
	...
	}, [highlightedFrame, overlayRect, screenHeight, screenWidth]);
				...
				{renderedEntries.map(({ element, index, key }) => {
					return (
						<AxTarget
							...
							selected={projectedSelectedKeys.has(key)}
