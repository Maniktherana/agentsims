...
function shortSourceLocation(
	...
	const shortPath =
		segments.length > 3 ? segments.slice(-3).join("/") : segments.join("/");
	const lineSuffix = typeof line === "number" && line > 0 ? `:${line}` : "";
	const columnSuffix =
		lineSuffix && typeof column === "number" && column >= 0 ? `:${column}` : "";
...
function sourceComponentBoundaryName(element: AxElement): string | null {
	...
	const directCustomBoundary =
		source.elementKind === "custom" ||
...
function nativeHostName(element: AxElement): string {
	...
	return element.source?.confidence !== "related-native-id" &&
...
export function accessibilityTreeRowTone(
	...
	if (
		accessibilityTreeRowAccessibleName(element) ||
		sourceComponentBoundaryName(element)
	) {
		return "content";
	}
...
export function accessibilityTreeProjectionStructureSignature(
	...
	return elements
		.map((element) => JSON.stringify([element.id, element.path]))
		.join("\u0001");
}
...
export function refreshAccessibilityTreeProjection(
	...
	const elementsByKey = new Map(
		elements.map((element) => [axElementKey(element), element]),
	);
...
export function isAccessibilityTreeRelevant(element: AxElement): boolean {
	...
	const role =
		`${element.role} ${element.type} ${element.traits?.join(" ") ?? ""}`.toLowerCase();
	...
	const directSource =
		element.source && element.source.confidence !== "related-native-id";
	return Boolean(
		directSource &&
...
function accessibilityTreeTooltipSourceBasename(
	element: AxElement,
): string | null {
	const file =
...
export function buildAccessibilityTreeProjection(
	...
	const visit = (
		...
		for (const projected of siblings) {
			...
			const baseSegment = treePathSegment(
				accessibilityTreeRowLabel(node.element),
			);
			const occurrence = (occurrences.get(baseSegment) ?? 0) + 1;
			occurrences.set(baseSegment, occurrence);
			const segment =
				occurrence === 1 ? baseSegment : `${baseSegment} · ${occurrence}`;
			const displayPath = parentPath ? `${parentPath}/${segment}` : segment;
			const canonicalPath =
				projected.children.length > 0 ? `${displayPath}/` : displayPath;
...
export function accessibilityTreeSearchResult(
	...
	for (const path of projection.paths) {
		...
		if (
			!entry ||
			!accessibilityTreeSearchText(entry.element).includes(normalized)
		) {
			continue;
		}
...
export function accessibilityTreeWindow(
	...
	const visibleStart = Math.floor(
		safeScrollTop / ACCESSIBILITY_TREE_ROW_HEIGHT_PX,
	);
	const visibleEnd =
		Math.ceil(
...
export function accessibilityTreeGuideSegments(
	rows: readonly Pick<FileTreeVisibleRow, "kind" | "isExpanded" | "level">[],
...
export function accessibilityTreeWindowGuideSegments(
	...
	const finish = (segment: OpenSegment) => {
		...
		segments.push({
			...
			top:
				(segment.drawStartRow - windowStartRow) *
				ACCESSIBILITY_TREE_ROW_HEIGHT_PX,
			height:
				(segment.endRow - segment.drawStartRow + 1) *
				ACCESSIBILITY_TREE_ROW_HEIGHT_PX,
		});
	};
	...
	return segments.sort(
		(left, right) => left.startRow - right.startRow || left.level - right.level,
	);
}
...
export function AccessibilityTree({
	...
	const visibleRowCount = useFileTreeSelector(model, (treeModel) =>
		treeModel.getVisibleCount(),
	);
	...
	const searchExpandedPathsSignature =
		searchResult.expandedPaths.join("\u0000");
	...
	useEffect(() => {
		...
		const expandedPaths = isSearching
			? [
					...new Set([
						...currentlyExpandedPaths.filter((path) => searchPathSet.has(path)),
	...
	const syncTreeViewport = useCallback((scroll = treeScrollRef.current) => {
		...
		const next = {
			...
			height:
				scroll.clientHeight || ACCESSIBILITY_TREE_INITIAL_VIEWPORT_HEIGHT_PX,
		};
	...
	const handleRowKeyDown = useCallback(
		(event: ReactKeyboardEvent<HTMLButtonElement>, row: FileTreeVisibleRow) => {
				...
				<>
					...
					>
						<div
							...
							style={{
								height: Math.max(rowWindow.totalHeight, treeViewport.height),
							}}
						>
							<div
							...
							>
								...
								{visibleRows.map((row, index) => {
									const entry = accessibilityTreeEntryForPath(
										projection,
										row.path,
									);
									...
									const tooltip =
										accessibilityTreeTooltipForPath(projection, row.path) ??
										label;
									const semanticLevel = row.level + 1;
									const layout = resolveAccessibilityTreeRowLayout(
										semanticLevel,
										220,
									);
									const rowHovered = hoveredTreeRow?.path === row.path;
									const rowHighlighted =
										rowHovered || highlightedKey === entry.key;
									const tabbable =
									...
									return (
										<button
											...
											onPointerEnter={(event) => {
												...
												if (host) {
													...
													const placeBelow =
														hostRect.bottom - rowRect.bottom >= 92;
													setHoveredTreeRow({
														path: row.path,
										...
										>
											...
											<span
												data-ax-tree-toggle={
													row.kind === "directory" ? "true" : undefined
												}
...
type SourceViewStyle = CSSProperties &
	Record<`--diffs-${string}`, string | number>;
...
function revealRenderedSourceLine(root: HTMLElement, line: number): boolean {
	const pierreHost = root.querySelector(
		"diffs-container",
	) as HTMLElement | null;
...
export function AccessibilitySourceSection({
	...
	useEffect(() => {
		...
		const reveal = () =>
			revealRenderedSourceLine(host, accessibilitySourceSelectedLine(excerpt));
		if (reveal()) return;
		const observer = new MutationObserver(() => {
			if (reveal()) observer.disconnect();
		});
		const pierreHost = host.querySelector(
			"diffs-container",
		) as HTMLElement | null;
...
function AccessibilityMetadataRow({
	...
	return (
		<div className="grid grid-cols-[76px_minmax(0,1fr)] gap-3 border-t border-white/[0.055] py-2.5 first:border-t-0">
			...
			<dd
				className={`m-0 min-w-0 break-words text-[11px] leading-4 text-white/70 ${code ? "font-mono" : ""}`}
			>
				{value}
			</dd>
		</div>
	...
	return (
		<>
			<AccessibilityMetadataRow
				label="Role"
				value={element.role || element.type}
			/>
			...
			<AccessibilityMetadataRow
				label="Bounds"
				value={axFrameString(element.frame)}
				code
			/>
...
function readAccessibilityMetadataHeight(): number {
	if (typeof window === "undefined")
		return ACCESSIBILITY_METADATA_DEFAULT_HEIGHT;
	const value = Number(
...
export function AccessibilityDetails({
	...
	useEffect(() => {
		if (!hasSourceMapping) {
			...
			setSourceState((current) =>
				current.status === "idle" ? current : { status: "idle", excerpt: null },
			);
			return;
		}
		const loader =
			sourceLoaderRef.current ??
		...
		return () => loader.cancel();
	}, [hasSourceMapping, sourceEndpoint, sourceFile, sourceLine, sourceTestID]);
	...
	const accessibleName = quotedAccessibleName(
		element,
	...
	const resizeMetadata = (
		...
		const available = Math.max(
			ACCESSIBILITY_METADATA_MIN_HEIGHT,
			(detailRef.current?.clientHeight ?? ACCESSIBILITY_METADATA_MAX_HEIGHT) -
				132,
		);
