...
/**
 * Low-overhead Android input channel backed by scrcpy's control protocol.
 * Video and audio are explicitly disabled: Agentsims' display stream remains
 * EmulatorController MMAP -> VideoToolbox H.264.
 */
export class AndroidScrcpyControlSession {
  ...
  private writeControl(message: Buffer): boolean {
  ...
  private async startImpl(): Promise<void> {
    ...
    this.control.once("error", () => this.close());
    this.control.once("close", () => this.close());
    this.server.close();
    this.server = null;
  }
}
