...
import type { AvccFrame } from "../ios/native";
...
interface AndroidVideoAddon {
  AndroidVideoCapture: new (path: string, onFrame: RawFrameCallback) => AndroidVideoCaptureHandle;
}
...
function resolveAddon(): string {
  ...
  const candidates = [
    ...(configuredDist ? [join(configuredDist, "native", "agentsims-android-video.node")] : []),
    ...
    join(moduleDirectory, "..", "..", "dist", "native", "agentsims-android-video.node"),
  ];
...
export class NativeAndroidVideoCapture {
  ...
  async subscribeAvcc(onFrame: (frame: AvccFrame) => Promise<void>): Promise<() => void> {
    if (this.handle) throw new Error("Android video capture already has a subscriber");
    const next = new (load().AndroidVideoCapture)(this.path, ([data, width, height, flags]) => {
      void onFrame({
        data,
        width,
        height,
