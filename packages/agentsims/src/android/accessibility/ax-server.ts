...
export class AndroidAxServerClient {
  ...
  snapshot(mode: AndroidAxMode = "fresh"): Promise<string> {
    if (mode === "latest" && this.latestXml) return Promise.resolve(this.latestXml);
  ...
  private ensureStarted(): Promise<void> {
    if (this.closed) return Promise.reject(new Error("Android AX server client is closed"));
    if (this.startPromise) return this.startPromise;
    if (this.child && !this.child.killed) return Promise.resolve();
    if (Date.now() < this.retryNotBefore) {
      return Promise.reject(new Error("Android AX server is cooling down after a failed start"));
    }
  ...
  private async startImpl(): Promise<void> {
    ...
    await adb(["-s", this.serial, "push", artifact, DEVICE_SERVER_PATH], 30_000);
    if (this.closed) throw new Error("Android AX server client closed during startup");
    ...
    child.once("exit", (code, signal) => {
      ...
      this.onChildExit(
        child,
        new Error(`Android AX server exited (${signal ?? code ?? "unknown"})${suffix}`),
      );
    });
  ...
  private onStdout(chunk: string): void {
    ...
    if (this.stdoutBuffer.length > MAX_PROTOCOL_BUFFER_BYTES) {
      this.failChild(new Error("Android AX server protocol buffer exceeded its limit"));
      return;
    }
    for (;;) {
      ...
      try {
        ...
        this.failChild(error instanceof Error ? error : new Error(String(error)));
        return;
      }
      if (response.ready) {
        this.ready?.resolve();
      ...
      if (response.event === "changed") {
        emitAndroidAxChange(this.serial, {
          ...
          eventTypes: Number.isFinite(response.eventTypes) ? response.eventTypes! : 0,
      ...
      if (response.ok && typeof response.xml === "string") {
        ...
        pending.reject(new Error(response.error || "Android AX snapshot failed"));
      }
    }
  }
