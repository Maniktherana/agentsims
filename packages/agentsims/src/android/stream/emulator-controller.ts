...
export function encodeSimulatorFrameTiming(
	sequence: number,
	timestampUs: number,
): Buffer {
...
function numericField(
	fields: Map<number, bigint | Uint8Array>,
	field: number,
): number {
	...
	if (encodedFormat instanceof Uint8Array) {
		...
		const rotation =
			encodedRotation instanceof Uint8Array
				...
				: 0;
		if (width && height)
			return { width, height, rotation, sequence, timestampUs };
	}
	return {
...
function orientation(
	width: number,
	height: number,
): AndroidEmulatorConfig["orientation"] {
...
type AndroidCaptureSink = Pick<
	NativeAndroidVideoCapture,
	"frame" | "requestKeyframe"
>;
...
export class AndroidAvccFrameCoordinator {
	...
	observeFrameMetadata(dimensions: {
		...
		if (
			...
			for (const subscriber of this.subscribers)
				this.writeSubscriber(subscriber, timing);
		}
	...
	setPresentationGeneration(generation: number): void {
		...
		this.lastPresentationMetadata =
			encodeAndroidPresentationMetadata(generation);
		for (const subscriber of this.subscribers) {
			subscriber.waitingForKeyframe = true;
			this.writeSubscriber(subscriber, this.lastPresentationMetadata);
			if (this.lastDescription)
				this.writeSubscriber(subscriber, this.lastDescription);
		}
	...
	attachSink(sink: AvccSubscriberSink): () => void {
		...
		const resume = () => {
			if (!this.subscribers.has(subscriber) || !subscriber.needsKeyframe)
				return;
	...
	publish(chunk: Buffer, isDescription = false): void {
		...
		for (const subscriber of this.subscribers)
			this.writeSubscriber(subscriber, chunk);
	}
...
export class AndroidEmulatorSession {
	...
	constructor(
		...
		this.mmapPath = join(
			tmpdir(),
			`agentsims-${this.metadata.pid}-${process.pid}.rgba`,
		);
	}
	...
	private async startImpl(): Promise<void> {
		...
		this.input = this.client.request(
			requestHeaders(INPUT_METHOD, this.metadata.token),
		);
		this.input.on("error", () => {
			this.input = null;
		});
		...
		await new Promise<void>((resolve, reject) => {
			...
			this.screenshots.on("response", (headers) => {
				if (headers[":status"] !== 200)
					fail(new Error(`Emulator gRPC HTTP ${headers[":status"]}`));
			});
			this.screenshots.on("data", (chunk: Buffer) => {
				...
				while (this.pendingGrpc.length >= 5) {
					...
					const dimensions = parseImageMetadata(
						this.pendingGrpc.subarray(5, 5 + length),
					);
			...
			this.screenshots.on("trailers", (trailers) => {
				if (trailers["grpc-status"] !== "0") {
					fail(
						new Error(
							String(
								trailers["grpc-message"] ?? "Emulator screenshot stream failed",
							),
						),
					);
				}
