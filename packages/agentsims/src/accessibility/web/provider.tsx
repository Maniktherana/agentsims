...
import type { AxSnapshot } from "../model";
import { openHostEventStream } from "../../web/utils/exec";
import { axElementKey, axElementsEqual, isAxeUnavailable } from "./ax";
import type {
...
export function useAxSnapshot(endpoint?: string, refreshSignal?: number) {
  ...
  const refresh = useCallback(async () => {
    ...
    setRefreshing(true);
    try {
      const response = await fetch(axRefreshEndpoint(endpoint), { method: "POST" });
      if (!response.ok) throw new Error(`AX refresh failed (${response.status})`);
    } catch {
  ...
  useEffect(() => {
    ...
    if (latestEndpointRef.current !== null && latestEndpointRef.current !== endpoint) {
    ...
    const connect = () => {
      if (disposed || source || (typeof document !== "undefined" && document.hidden)) return;
      source = openHostEventStream(endpoint);
      source.onmessage = (event) => {
        setRefreshing(false);
        try {
          const next = decodeAxSnapshotEvent(event.data, latestPayloadRef.current);
          if (!next) {
            setStatus((current) =>
              current === latestStatusRef.current ? current : latestStatusRef.current
            );
            return;
          }
          latestPayloadRef.current = next.payload;
          latestStatusRef.current = next.status;
          const reconciled = reconcileAxSnapshot(latestSnapshotRef.current, next.snapshot);
          ...
          setStatus((current) => current === "AX parse error" ? current : "AX parse error");
        }
      };
      source.onerror = () => {
        setStatus((current) => current === "AX reconnecting" ? current : "AX reconnecting");
      };
    };
...
export function AccessibilityStateProvider({
  ...
  const snapshotValue = useAxSnapshot(state.open ? endpoint : undefined, refreshSignal);
  ...
  const setSelectedKey = useCallback((key: string | null, origin: "phone" | "tree" = "tree") => {
  ...
  const selectionValue = useMemo<AxSelectionContextValue>(() => ({
    ...
    setHighlightedKey,
    setSelectedKey,
  }), [setHighlightedKey, setSelectedKey, state.highlightedKey, state.highlightedOrigin, state.selectedKey]);
