...
import { useAccessibilityPanelPosition } from "./panel-position";
...
export function AccessibilityInspectorController({
  ...
  const panelPosition = useAccessibilityPanelPosition(anchor, state.open, deviceId);
  ...
  const selectedElement = selectedKey
    ...
    : null;
  const nativeChain = selectedKey && snapshot
    ? accessibilityNativeChain(snapshot.elements, selectedKey)
    : [];

  const panel = state.open && focused
    ? createPortal(
        <div
