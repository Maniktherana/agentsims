...
async function resolveDeviceId(
  ...
  const selected = status.workspaces?.find((workspace) => workspace.device)?.device;
  if (!selected) {
    throw new Error(
...
export async function readAccessibilityTree(device?: string, origin?: string): Promise<unknown> {
  const commandClient = client(origin);
  const deviceId = await resolveDeviceId(commandClient, device);
  const observation = await commandClient.observeDevice(deviceId, true) as DeviceObservation;
  if (observation.accessibility === null) {
    throw new Error(observation.warnings[0] ?? "Accessibility data is not available");
  }
...
export async function observeDevice(options: {
  ...
  const screenshot = Buffer.from(observation.screenshot.contentBase64, "base64");
  if (screenshot.byteLength !== observation.screenshot.bytes) {
    throw new Error("The observation screenshot length does not match its metadata");
  }
...
export async function gesture(json: string, device?: string, origin?: string): Promise<void> {
...
export async function tap(
  ...
  await actOnDevice(
    parseAgentAction(JSON.stringify({ type: "tap", x: Number(xValue), y: Number(yValue) })),
    device,
    origin,
  );
}
