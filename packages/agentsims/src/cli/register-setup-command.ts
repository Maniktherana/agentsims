...
export async function runSetupCommand(
  ...
  if (plan.status === "already-configured") {
    io.stdout.write(`Agentsims source mapping is already configured in ${plan.configPath}.\n`);
    return;
  }
  ...
  const applied = system ? applyMetroSetup(plan, system) : applyMetroSetup(plan);
  io.stdout.write(`Configured Agentsims source mapping in ${applied.configPath}.\n`);
