...
export function deviceHelp(
	...
	if (group === "input") {
		if (action === "tap")
			return `Usage: agentsims device ${device} input tap <x> <y>\n`;
		if (action === "button")
			return `Usage: agentsims device ${device} input button [name]\n`;
		if (action === "type")
			return `Usage: agentsims device ${device} input type [text...]\n`;
		if (action === "rotate") {
...
async function runCameraCommand(
	...
	switch (action) {
		...
		case "back":
			printJson(
				await appClient.applyMedia(device, {
...
export function addWorkspaceCommands(
	...
	const devices = program
		.command("devices")
		.description("Manage the device catalog");
	devices
		.command("list")
