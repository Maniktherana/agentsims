...
function explicitProjectRoot(path: string): string {
  ...
  if (!existsSync(projectRoot) || !statSync(projectRoot).isDirectory()) {
    throw new MetroSetupError(`Project directory does not exist: ${projectRoot}`);
  }
  return projectRoot;
}

function findConfig(projectRoot: string, explicitConfig?: string): string | null {
  if (explicitConfig) {
    const path = isAbsolute(explicitConfig)
      ? explicitConfig
      : resolve(projectRoot, explicitConfig);
    if (!existsSync(path)) throw new MetroSetupError(`Metro config does not exist: ${path}`);
    return path;
  }

  const supported = ROOT_CONFIG_NAMES
    .map((name) => join(projectRoot, name))
    .filter(existsSync);
  const unsupported = UNSUPPORTED_CONFIG_NAMES
    .map((name) => join(projectRoot, name))
    .filter(existsSync);
  const nested = ROOT_CONFIG_NAMES
    .map((name) => join(projectRoot, ".config", name.replace("metro.config", "metro")))
    .filter(existsSync);
  const packagePath = join(projectRoot, "package.json");
  const hasPackageConfig =
    existsSync(packagePath) && Object.hasOwn(parseJsonFile(packagePath), "metro");

  if (supported.length === 2) {
    const shim = supported.find((path) => [".js", ".cjs"].includes(extname(path)));
    if (shim) {
...
function expoTypeScriptShimTarget(path: string): string | null {
  ...
  return [".ts", ".cts", ".mts"].includes(extname(resolvedTarget)) && existsSync(resolvedTarget)
    ? resolvedTarget
    : null;
}
...
function parseConfig(source: string, path: string): any {
  ...
  try {
    ...
    throw new MetroSetupError(`Could not parse ${path}: ${details}\nNo files changed.`);
  }
}
  ...
  for (const statement of program.body as any[]) {
    if (statement.type === "ImportDeclaration") {
      ...
      for (const specifier of statement.specifiers as any[]) {
        if (specifier.type === "ImportSpecifier") {
          ...
          if (imported) result.named.set(specifier.local.name, `${source}#${imported}`);
          if (source === AGENTSIMS_METRO && imported === "withAgentsims") {
            result.agentsimsBinding = { kind: "identifier", name: specifier.local.name };
          }
        } else if (specifier.type === "ImportNamespaceSpecifier") {
          result.namespaces.set(specifier.local.name, source);
          if (source === AGENTSIMS_METRO) {
            result.agentsimsBinding = { kind: "namespace", name: specifier.local.name };
          }
        }
      }
    ...
    for (const declaration of statement.declarations as any[]) {
      ...
      if (declaration.id.type === "Identifier") {
        ...
        if (source === AGENTSIMS_METRO) {
          result.agentsimsBinding = { kind: "namespace", name: declaration.id.name };
...
function calleeModuleExport(callee: any, bindings: ModuleBindings): string | null {
  const expression = unwrapExpression(callee);
  if (expression?.type === "Identifier") return bindings.named.get(expression.name) ?? null;
  if (
...
function isAgentsimsCall(node: any, bindings: ModuleBindings): boolean {
  ...
  return (
    expression?.type === "CallExpression" &&
    calleeModuleExport(expression.callee, bindings) === `${AGENTSIMS_METRO}#withAgentsims`
  );
}
...
function containsAgentsimsCall(node: any, bindings: ModuleBindings): boolean {
  ...
  for (const [key, value] of Object.entries(node)) {
    ...
    if (Array.isArray(value)) {
      if (value.some((entry) => containsAgentsimsCall(entry, bindings))) return true;
...
function hasExplicitTransformer(node: any): boolean {
  ...
  const transformer = expression.properties.find(
    (property: any) =>
      property.type === "ObjectProperty" && propertyName(property.key) === "transformer",
  );
  ...
  const babelPath = transformerValue.properties.find(
    (property: any) =>
      property.type === "ObjectProperty" && propertyName(property.key) === "babelTransformerPath",
  );
...
function isSynchronousMergeArgument(
  ...
  if (expression.type === "Identifier") {
    ...
    return isSynchronousMergeArgument(initializer, moduleBindings, initializers, nextSeen);
  }
  ...
  if (source && (CONFIG_COMBINERS.has(source) || CONFIG_WRAPPERS.has(source))) {
    return resolvesMetroConfig(expression, moduleBindings, initializers, new Set(seen));
  }
  return false;
}
...
function resolvesMetroConfig(
  ...
  if (expression.type === "Identifier") {
    ...
    const nextSeen = new Set(seen);
    nextSeen.add(expression.name);
    return resolvesMetroConfig(initializer, moduleBindings, initializers, nextSeen);
  }
  ...
  if (CONFIG_COMBINERS.has(source)) {
    return (
      ...
      expression.arguments.some((argument: any) =>
        resolvesMetroConfig(argument, moduleBindings, initializers, new Set(seen)),
      )
    );
  }
  if (CONFIG_WRAPPERS.has(source)) {
    return resolvesMetroConfig(expression.arguments[0], moduleBindings, initializers, new Set(seen));
  }
  return false;
}
...
function insertionForNewImport(
  ...
  let offset = program.interpreter ? lineEnd(source, program.interpreter.end) : 0;
  const directives = program.directives as any[] | undefined;
  if (directives?.length) offset = lineEnd(source, directives[directives.length - 1]!.end);

  if (style === "esm") {
    const imports = (program.body as any[]).filter((node) => node.type === "ImportDeclaration");
    if (imports.length) offset = lineEnd(source, imports[imports.length - 1]!.end);
  } else {
    for (const statement of program.body as any[]) {
...
function editExistingAgentsimsImport(
  ...
  if (importNode.type === "ImportDeclaration") {
    const named = importNode.specifiers.filter((specifier: any) => specifier.type === "ImportSpecifier");
    if (named.length === 0) {
      throw new MetroSetupError(
...
function bindingCall(binding: ImportBinding): string {
  return binding.kind === "identifier" ? binding.name : `${binding.name}.withAgentsims`;
}
...
function applySourceEdits(source: string, edits: SourceEdit[]): string {
  ...
  for (const edit of sorted) {
    if (edit.start < 0 || edit.end < edit.start || edit.end > source.length) {
      throw new MetroSetupError("An internal setup edit was outside the Metro config bounds.");
    }
    if (edit.end > previousStart) {
      throw new MetroSetupError("Internal setup edits overlapped; no files changed.");
    }
...
export function transformMetroConfig(source: string, path: string): TransformedMetroConfig {
  ...
  const resolvedExpression = resolveTopLevelInitializer(expression, initializers);
  ...
  if (typeof target.expression.start !== "number" || typeof target.expression.end !== "number") {
    throw new MetroSetupError(`Could not locate the exported value in ${path}.\nNo files changed.`);
  }

  const edits: SourceEdit[] = [];
  let binding = moduleBindings.agentsimsBinding;
  if (!binding && moduleBindings.agentsimsImport) {
    const existing = editExistingAgentsimsImport(source, moduleBindings.agentsimsImport);
  ...
  if (!binding) {
    ...
    const importStyle =
      target.style === "esm" ||
      (program.body as any[]).some((statement) => statement.type === "ImportDeclaration")
        ? "esm"
        : "cjs";
    const insertion = insertionForNewImport(source, program, importStyle);
    edits.push({ start: insertion.offset, end: insertion.offset, text: insertion.text });
  }

  const call = bindingCall(binding);
  edits.push(
    { start: target.expression.start, end: target.expression.start, text: `${call}(` },
  ...
  if (verification.status !== "already-configured") {
    throw new MetroSetupError(`Could not verify the generated Metro setup for ${path}.`);
  }
...
function assertRegularConfig(path: string): void {
  ...
  if (!stat.isFile()) {
    throw new MetroSetupError(`${path} is not a regular file.\nNo files changed.`);
  }
}
...
export function planMetroSetup(
  ...
  const projectRoot = input.project
    ? explicitProjectRoot(isAbsolute(input.project) ? input.project : resolve(start, input.project))
  ...
  const detectedFramework =
    framework ??
    (original.includes("expo/metro-config") || original.includes("getSentryExpoConfig")
      ? "expo"
      : "react-native");
  return {
    projectRoot,
...
function assertExpectedFileState(path: string, expectedHash: string | null | undefined): void {
...
function atomicWrite(
  ...
  const temporary = join(dirname(path), `.${basename(path)}.agentsims-${process.pid}-${randomUUID()}.tmp`);
...
function verifyWrittenConfig(path: string): void {
  ...
  if (transformed.status !== "already-configured") {
    throw new MetroSetupError(`The written Metro config did not pass setup verification: ${path}`);
  }
}
...
export function applyMetroSetup(
  ...
  if (existed) {
    if (!existsSync(plan.configPath)) {
      throw new MetroSetupError(`${plan.configPath} changed after setup was planned. No files changed.`);
    }
    ...
    throw new MetroSetupError(`${plan.configPath} was created after setup was planned. No files changed.`);
  }
  ...
  try {
    ...
    try {
      if (backupPath) {
        ...
        if (sha256(readFileSync(plan.configPath, "utf8")) !== sha256(plan.updated)) {
          throw new MetroSetupError(
