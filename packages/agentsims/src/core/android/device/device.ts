...
import type { AxElement, AxRect, AxSnapshot } from "../../tools/observe/accessibility-model";
import type {
	AndroidAudioStatus,
...
async function getProp(
	...
	try {
		const value = (
			await adbText(["-s", serial, "shell", "getprop", name], 3_000)
		).trim();
...
export function validateAndroidCameraStartupMode(
	...
	if (source === "none" || source === "emulated" || source === "environment")
		return true;
