...
import { deviceCatalog, type GridDevice, type GridPage, type MemoryReport } from "../server/devices/device-catalog";
...
type DeviceCatalogCommands = Pick<typeof deviceCatalog, "page" | "memoryReport">;
...
export class DeviceCommands {
  ...
  list(options: DeviceListOptions = {}): Effect.Effect<GridPage, ApplicationCommandError> {
    return Effect.tryPromise({
      try: () => this.catalog.page({
  ...
  status(deviceId: string): Effect.Effect<GridDevice | null, ApplicationCommandError> {
    return this.list().pipe(
      Effect.map((page) => page.devices.find((device) => device.device === deviceId) ?? null),
    );
  }
  ...
  start(
    ...
    return Effect.tryPromise({
      try: () => this.lifecycle.start(deviceId, options.port, options.basePath ?? "/"),
      catch: commandFailure,
    }).pipe(
