...
import { commandFailure, InvalidCommandInput, type ApplicationCommandError } from "./errors";
...
export class DeviceActionCommands {
  ...
  act(
    ...
    return Effect.gen(this, function*() {
      if (!device) return yield* Effect.fail(new InvalidCommandInput({ message: "Invalid or missing device" }));
      if (values.length === 0) {
        return yield* Effect.fail(new InvalidCommandInput({ message: "At least one action is required" }));
      }
