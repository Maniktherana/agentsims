...
export const DeviceListInputSchema = Schema.Struct({
  ...
  limit: Schema.optional(Schema.NullOr(Schema.Number.pipe(Schema.nonNegative()))),
  offset: Schema.optional(Schema.Number.pipe(Schema.nonNegative())),
});

export const DeviceStatusInputSchema = Schema.Struct({ device: DeviceIdSchema });
...
export const DeviceCommandSuccessSchema = Schema.Struct({ ok: Schema.Literal(true) });
export const DeviceStartOutputSchema = Schema.Struct({ device: DeviceIdSchema });
