...
function emulatorController() {
  ...
  if (!ini) throw new Error("No running Android emulator controller metadata found");
...
function processCpu(pid) {
  return Number(execFileSync("ps", ["-p", String(pid), "-o", "%cpu="], { encoding: "utf8" }).trim());
}
...
const mmapPath = mmap ? `/private/tmp/agentsims-grpc-${process.pid}.rgba` : null;
if (mmapPath) {
...
const input = drive ? client.request({
  ":method": "POST",
...
request.on("data", (chunk) => {
  ...
  while (pending.length >= 5) {
    ...
    if (compressed !== 0) throw new Error("Compressed gRPC messages are unsupported");
...
setTimeout(() => {
  ...
  console.log(JSON.stringify({
    ...
    emulatorCpu: {
      average: Number((cpu.reduce((sum, value) => sum + value, 0) / cpu.length).toFixed(1)),
      max: Math.max(...cpu),
      samples: cpu,
    },
    first,
