...
const geomToLL = (el: any): LL[] =>
	el.geometry.map((p: any) => [p.lat, p.lon] as LL);
...
log(
	`  MD pts=${md.length} ${JSON.stringify(md[0])}->${JSON.stringify(md[md.length - 1])}`,
);
log(
	`  SR pts=${sr.length} ${JSON.stringify(sr[0])}->${JSON.stringify(sr[sr.length - 1])}`,
);
...
const hwy_chain = chains.length
	? chains.reduce((a, b) => (a.length >= b.length ? a : b))
	: [];
log(`  longest chain pts=${hwy_chain.length}`);
if (hwy_chain.length) {
	log(
		`    ${JSON.stringify(hwy_chain[0])} -> ${JSON.stringify(hwy_chain[hwy_chain.length - 1])}`,
	);
}
