...
function touch(type: "begin" | "move" | "end", x: number, y: number): Uint8Array {
...
function startCpuSampler(pid: string | null, samples: number[]): () => void {
  ...
  const timer = setInterval(() => {
    if (running) return;
    running = true;
    execFile("ps", ["-p", pid, "-o", "%cpu="], { encoding: "utf8" }, (error, stdout) => {
      running = false;
      if (error) return;
...
const activeFrames = mediaFrameTimes.filter((at) => at >= activeStart && at <= activeEnd).length;
...
if (!counts.description || !counts.keyframe || gestures < 2 || observedFps < 30) {
  process.exitCode = 1;
}
