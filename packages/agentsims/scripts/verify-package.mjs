...
function packagePath(relativePath) {
	assert.match(
		relativePath,
		/^\.\//,
		`export target must be package-relative: ${relativePath}`,
	);
...
function assertFile(relativePath) {
	...
	assert.ok(
		...
		`missing package artifact: ${relativePath}`,
	);
	return absolutePath;
}
...
assert.notEqual(
	manifest.version,
	"0.0.0",
	"publish version must not be the placeholder",
);
...
const previewAssets = readdirSync(
	join(packageRoot, "dist", "preview", "assets"),
);
assert.ok(
...
assert.equal(
	readFileSync(binPath, "utf8").split(/\r?\n/, 1)[0],
	"#!/usr/bin/env bun",
);
...
for (const [subpath, conditions] of Object.entries(manifest.exports ?? {})) {
	assert.equal(
		typeof conditions,
		"object",
		`invalid exports entry: ${subpath}`,
	);
	for (const [condition, target] of Object.entries(conditions)) {
		assert.equal(
			typeof target,
			"string",
			`invalid ${subpath} ${condition} target`,
		);
		assert.ok(
			target.startsWith("./dist/"),
			`${subpath} ${condition} must use a built artifact`,
		);
		assertFile(target);
...
for (const conditions of Object.values(manifest.exports ?? {})) {
	...
	if (conditions.import)
		await import(pathToFileURL(packagePath(conditions.import)).href);
}
...
for (const artifact of [
	...
	execFileSync(
		"lipo",
		[join(packageRoot, artifact), "-verify_arch", "x86_64", "arm64"],
		{
			stdio: "pipe",
		},
	);
...
const androidVideoAddon = join(
	packageRoot,
	"dist/native/agentsims-android-video.node",
);
const installName = execFileSync("otool", ["-D", androidVideoAddon], {
	encoding: "utf8",
})
	.split(/\r?\n/)
	.slice(1)
...
assert.ok(
	!source.includes(packageRoot),
	"dist/agentsims.js contains the build-machine package path",
);
